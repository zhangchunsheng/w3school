...
<div>
<h2>定义和用法</h2>
<p>previousSibling 属性可返回某节点之前紧跟的节点（处于同一树层级）</p>
<p>如果没有此节点，那么该属性返回 null。</p>

<h3>语法：</h3>
<pre>nodeObject.previousSibling</pre>
</div>

<div>
<h2>提示和注释</h2>

<p class="note"><span>注释：</span>Internet Explorer 会忽略节点间生成的空白文本节点（例如，换行符号），而 Mozilla 不会这样做。因此，在下面的例子中，我们会使用一个函数来检查首个子节点的节点类型。</p>
<p>元素节点的节点类型是 1，因此如果首个子节点不是一个元素节点，它就会移至下一个节点，然后继续检查此节点是否为元素节点。整个过程会一直持续到首个元素子节点被找到为止。通过这个方法，我们就可以在 Internet Explorer 和 Mozilla 得到正确的方法。</p>
...
<p>下面的代码片段可从XML文档中 &lt;author&gt; 元素取得前一个同级节点：</p>
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_previoussibling">previousSibling - 取得某节点的上一个同级节点</a></dt>
<dt><a href="/tiy/t.asp?f=xdom_nextsibling2">nextSibling - 取得某节点的下一个同级节点</a></dt>
</dl>
</div>
