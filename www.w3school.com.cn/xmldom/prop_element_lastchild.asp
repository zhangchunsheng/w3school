...
<p>lastChild 属性返回被选节点的最后一个子节点。</p>

<p>如果选定的节点没有子节点，则该属性返回 NULL。</p>

<h3>语法：</h3>
<pre>elementNode.lastChild</pre>
</div>

<div>
<h2>提示和注释</h2>
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点之间生成的空白文本节点（比如换行字符），而 Mozilla 不这么做。因此，在下面的例子中，我们用一个函数来检测最后一个子节点的节点类型。</p>

<p>元素节点的节点类型是 1，因此假如第一个子节点不是元素节点，则移动到下一个节点，并检测该节点是否是元素节点。这个过程一直持续到找到第一个子节点为止。这种方法可以确保在 Internet Explorer 和 Mozilla 都获得正确的结果。</p>
...
<p>下面的代码片段从 &quot;books.xml&quot; 中获取最后一个子节点：</p>

<pre>//check if the last node is an element node
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_lastchild">lastChild - 获取 XML 文档中的最后一个子节点</a></dt>
</dl>
</div>
