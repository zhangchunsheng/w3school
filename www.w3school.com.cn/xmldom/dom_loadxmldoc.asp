...
<li class="currentLink"><a href="/xmldom/dom_loadxmldoc.asp" title="XML DOM 加载函数">DOM 加载</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/xmldom/dom_parser.asp" title="解析 XML DOM">DOM 解析</a></li>
<li class="next"><a href="/xmldom/dom_methods.asp" title="XML DOM - 属性和方法">DOM 属性和方法</a></li>
</ul>
</div>

<div id="intro">
<p><strong>可以把供加载 XML 文档的代码存储在单独的函数中。</strong></p>
</div>

<div class="example">
<h2>实例</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_parser">加载 XML 文档</a></dt>
<dt><a href="/tiy/t.asp?f=xdom_loadxmldoc">使用函数来加载 XML 文档</a></dt>
</dl>
</div>
...
<p>XML DOM 含有遍历 XML 树以及访问、插入、删除节点的方法（函数）。</p>

<p>然后，在访问并处理 XML 文档之前，必须把它载入 XML DOM 对象。</p>

<p>上一节演示了如何加载 XML 文档。为了避免因加载文档而重复编写代码，可以把代码存储在一个单独的 JavaScript 文件中：</p>

<pre>function loadXMLDoc(dname) 
{
try <span>//Internet Explorer</span>
  {
  xmlDoc=new ActiveXObject(&quot;Microsoft.XMLDOM&quot;);
  }
catch(e)
  {
  try <span>//Firefox, Mozilla, Opera, etc.</span>
    {
...
<p>上面的函数存储在名为 &quot;loadxmldoc.js&quot; 的文件中。</p>

<p>下面的例子在其 &lt;head&gt; 部分有一个指向 &quot;loadxmldoc.js&quot; 的链接，并使用 loadXMLDoc() 函数加载 XML 文档 (&quot;books.xml&quot;)：</p>
...
<p><a href="/tiy/t.asp?f=xdom_loadxmldoc">TIY</a></p>
</div>
