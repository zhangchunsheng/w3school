...
<title>XML DOM getElementsByTagName() 方法</title>
</head>
...
<h1>XML DOM getElementsByTagName() 方法</h1>
...
<div>
<h2>定义和用法</h2>
<p>getElementsByTagName() 方法可返回带有指定名称的所有元素的一个节点列表。</p>

<h3>语法：</h3>
<pre>getElementsByTagName(<i>name</i>)</pre>
...
<tr>
<td>name</td>
<td>字符串值，规定需检索的标签名。值 &quot;*&quot; 匹配所有的标签。</td>
</tr>
</table>
...
<p>文档树中具有指定标记的 Element 节点的只读数组（从技术上讲，是 <a href="/xmldom/dom_nodelist.asp" title="XML DOM - NodeList 对象">NodeList 对象</a>）。返回的元素节点的顺序就是它们在源文档中出现的顺序。</p>


<h3>说明</h3>

<p>该方法将返回一个 <a href="/xmldom/dom_nodelist.asp" title="XML DOM - NodeList 对象">NodeList 对象</a>（可以作为只读数组处理），该对象存放文档中具有指定标签名的所有 Element 节点，它们存放的顺序就是在源文档中出现的顺序。<a href="/xmldom/dom_nodelist.asp" title="XML DOM - NodeList 对象">NodeList 对象</a>是“活”的，即如果在文档中添加或删除了指定标签名的元素，它的内容会自动进行必要的更新。</p>

<p>注意，Element 接口定义了一个同名的方法，该方法只检索文档的子树。另外，HTMLDocument 接口定义了 <a href="/jsref/met_doc_getelementsbyname.asp" title="HTML DOM getElementsByName() 方法">getElementsByName() 方法</a>，基于 name 属性的值（而不是标签名）检索元素。</p>


<h3>例子</h3>
<p>可以用下列代码检索并遍历 HTML 文档中的所有 &lt;h1&gt; 标记：</p>
<pre>var headings = document.<code>getElementsByTagName</code>(&quot;h1&quot;);
...
<p>以下代码片段可显示 &quot;books.xml&quot; 中所有 &lt;title&gt; 元素的值：</p>
<pre>
xmlDoc=loadXMLDoc(&quot;/example/xdom/books.xml&quot;);

var x=<code>xmlDoc.getElementsByTagName('title')</code>;
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_getelementsbytagname">getElementsByTagName() - 取得指定节点的值</a></dt>
</dl>
</div>
