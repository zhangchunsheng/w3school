...
<title>XML DOM insertBefore() 方法</title>
</head>
...
<p>insertBefore() 方法在已有的子节点之前插入一个新的子节点。</p>

<p>该方法返回这个新的子节点。</p>
<h3>语法：</h3>
<pre>elementNode.insertBefore(new_node,existing_node)</pre>
...
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点之间生成的空白文本节点（比如换行字符），而 Mozilla 不这么做。因此，在下面的例子中，我们使用一个函数来检查最后一个子节点的节点类型。</p>

<p>元素节点的节点类型是 1，因此如果最后一个子节点不是元素节点，则移动到上一个节点，并检查这个节点是否是元素节点。这个过程一直会持续到找到最后一个属于元素节点的子节点为止。通过这个方法，在 Internet Explorer 和 Mozilla 中都会得到正确的结果。</p>
...
<p>下面的代码片段创建一个新的 &lt;book&gt; 节点，并把它插到文档中最后一个 &lt;book&gt; 元素之前：</p>

<pre>//check if the last childnode is an element node
...
<code>xmlDoc.documentElement.insertBefore(newNode,get_lastchild(x));</code></pre>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_insertbefore">insertBefore() - 在指定节点之前添加节点</a></dt>
<dt><a href="/tiy/t.asp?f=xdom_element_insertbefore">insertBefore() - 在指定子节点之前添加节点</a></dt>
</dl>
</div>
