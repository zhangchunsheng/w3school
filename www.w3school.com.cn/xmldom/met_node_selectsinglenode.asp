...
<title>XML DOM selectSingleNode() 方法</title>
</head>
...
<h1>XML DOM selectSingleNode() 方法</h1>
...
<div>
<h2>定义和用法</h2>
<p>selectSingleNode() 方法查找和 XPath 查询匹配的一个节点。</p>

<h3>语法：</h3>
<pre>nodeObject.selectSingleNode(query)</pre>
...
<h3>返回值</h3>
<p>匹配查询的一个单独的 Node。如果没有，则为 null。</p>

<h3>说明</h3>
<p>这个特定于 IE 的方法计算一个 XPath 表达式，使用这个节点作为 context 节点。它返回找到的第一个匹配节点，如果没有匹配的节点就返回 null。</p>
<p>这个 selectSingleNode() 方法只在 XML 文档节点上存在，在 HTML 文档中不存在。注意，既然 <a href="/xmldom/dom_document.asp" title="XML DOM - Document 对象">Document 对象</a>是它们自己的节点，这个方法可以应用于整个 XML 文档。</p>

<p>参阅 <a href="/xmldom/met_document_evaluate.asp" title="XML DOM evaluate() 方法">Document.evaluate()</a>，了解一个跨浏览器的替代方法。</p>

<h3>参阅</h3>
<p><a href="/xmldom/met_document_createexpression.asp" title="XML DOM createExpression() 方法">Document.createExpression()</a></p>
</div>
