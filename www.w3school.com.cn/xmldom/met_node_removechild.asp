...
<title>XML DOM removeChild() 方法</title>
</head>
...
<h3>语法：</h3>
<pre>nodeObject.removeChild(node)</pre>
...
<div>
<h2>提示和注释</h2>
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点间生成的空白文本节点（例如，换行符号），而 Mozilla 不会这样做。因此，在下面的例子中，我们会使用一个函数来检查最后一个子节点的节点类型。</p>
<p>元素节点的节点类型是1，因此如果首个子节点不是一个元素节点，它就会移至下一个节点，然后继续检查此节点是否为元素节点。整个过程会一直持续到首个元素子节点被找到为止。通过这个方法，我们就可以在 Internet Explorer 和 Mozilla 得到正确的方法。</p>
...
<p>下面的代码片段可删除首个 &lt;book&gt; 元素中的最后一个子节点：</p>

<pre>//check if last child node is an element node
...
deleted_node=<code>x.removeChild(get_lastchild(x))</code>;
document.write(&quot;Node removed: &quot; + deleted_node.nodeName);</pre>

<h3>输出：</h3>
<pre>Node removed: price</pre>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_removechild">removeChild() - 从节点列表删除最后一个子节点</a></dt>
</dl>
</div>
