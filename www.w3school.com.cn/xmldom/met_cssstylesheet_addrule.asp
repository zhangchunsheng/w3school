...
<title>XML DOM addRule() 方法</title>
</head>
...
<div>
<h2>定义和用法</h2>
<p>addRule() 方法向样式表插入一条规则，该方式是特定于 IE 的方法。</p>

<h3>语法：</h3>
<pre>addRule(<i>selector</i>,<i>style</i>,<i>index</i>)</pre>
...
<tr>
<td>selector</td>
<td>必需。规则的 CSS 选择器。</td>
</tr>

<tr>
<td>style</td>
<td>
	<p>必需。应用于匹配该选择器的元素的样式。</p>
	<p>这个样式字符串是一个分号隔开的属性：值对的列表。并没有使用花括号开始或结束。</p>
</td>
</tr>
...
<td>index</td>
<td>
	<p>可选。规则数组中插入或附加规则的位置。</p>
	<p>如果这个可选参数被省略掉，则新的规则会增加到规则数组的最后。</p>
</td>
</tr>
</table>
...
<p>该方法在样式表的 rules 数组的指定 <i>index</i> 处插入（或附加）一条新的 CSS 样式规则。这是标准 insertRule() 方法的特定于 IE 的替代。</p>

<p class="important"><span>注意：</span>这个方法的参数和 insertRule() 方法的参数不同。</p>
</div>
...
<p>XML DOM 参考手册：<a href="/xmldom/met_cssstylesheet_insertrule.asp">CSSStyleSheet.insertRule()</a></p>
</div>
