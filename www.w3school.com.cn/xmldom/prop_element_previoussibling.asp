...
<title>XML DOM previousSibling 属性</title>
</head>
...
<h1>XML DOM previousSibling 属性</h1>
...
<p>previousSibling 属性返回选定节点的上一个同级节点（在相同树层级中的前一个节点）。</p>

<p>如果不存在这样的节点，则该属性返回 null。</p>

<h3>语法：</h3>
<pre>elementNode.previousSibling</pre>
</div>

<div>
<h2>提示和注释</h2>
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点之间生成的空白文本节点（比如换行字符），而 Mozilla 不这么做。因此，在下面的例子中，我们用一个函数来检测上一个同级节点的节点类型。</p>
<p>元素节点的节点类型是 1，因此假如上一个同级节点不是元素节点，则移动到上一个节点，并检测该节点是否是元素节点。这个过程一直持续到找到上一个同级节点为止。这种方法可以确保在 Internet Explorer 和 Mozilla 都获得正确的结果。</p>
<p class="tip">如需更多有关 IE 与 Mozilla 浏览器差异的内容，请访问 W3School 的 XML DOM 教程中的 <a href="dom_mozilla_vs_ie.asp" title="XML DOM 浏览器差异">DOM 浏览器</a> 这一节。</p>
</div>
...
<p>下面的代码片段获取 XML 文档中第一个 &lt;author&gt; 元素的上一个同级节点：</p>

<pre>//check if the previous sibling node is an element node
function get_previoussibling(n)
{
var x=n.previousSibling;
while (x.nodeType!=1)
  {
  x=<code>x.previousSibling</code>;
  }
return x;
}
...
document.write(&quot; = &quot;);
document.write(y.childNodes[0].nodeValue);</pre>

<p>以上代码的输出：</p>

<pre>author = Giada De Laurentiis
Previous sibling: title = Everyday Italian</pre>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_nextsibling2">nextSibling - 获取节点的下一个同级节点</a></dt>
<dt><a href="/tiy/t.asp?f=xdom_previoussibling">previousSibling - 获取节点的上一个同级节点</a></dt>
</dl>
</div>
