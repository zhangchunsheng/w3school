...
<p>removeChild() 方法删除子节点。</p>

<p>如成功，则返回被删除的节点，否则返回 NULL。</p>

<h3>语法：</h3>
<pre>elementNode.removeChild(node)</pre>
...
<p>下面对代码片段删除第一个 &lt;book&gt; 元素中最后一个子节点：</p>
...
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点间生成的空白文本节点（例如，换行符号），而 Mozilla 不会这样做。因此，在上面的例子中，我们创建了一个函数来获取正确的子元素。</p>

<p class="tip"><span>提示：</span>如需更多有关 IE 与 Mozilla 浏览器差异的内容，请访问 W3School 的 XML DOM 教程中的 <a href="dom_mozilla_vs_ie.asp" title="XML DOM 浏览器差异">DOM 浏览器</a> 这一节。</p>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_removechild">removeChild() - 从 nodelist 中删除最后一个子节点</a></dt>
</dl>
</div>
