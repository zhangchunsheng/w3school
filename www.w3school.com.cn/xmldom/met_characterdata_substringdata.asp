...
<title>XML DOM substringData() 方法</title>
</head>
...
<h1>XML DOM substringData() 方法</h1>

<div class="backtoreference">
<p><a href="/xmldom/dom_characterdata.asp" title="XML DOM CharacterData 对象">CharacterData 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>substringData() 方法从 <a href="/xmldom/dom_text.asp">Text</a> 或 <a href="/xmldom/met_text_appenddata.asp">Comment</a> 节点中提取子串。</p>

<h3>语法：</h3>
<pre>CharacterData.substringData(<i>start</i>,<i>length</i>)</pre>
...
<tr>
<td>start</td>
<td>必需。要返回的第一个字符的位置。</td>
</tr>

<tr>
<td>length</td>
<td>必需。要返回的子串中的字符数。</td>
</tr>
</table>

<h3>返回值</h3>

<p>返回一个字符串，包含 Text 或 Comment 节点中从 <i>start</i> 开始的 <i>length</i> 个字符。</p>

<h3>抛出</h3>

<p>该方法可以抛出具有以下代码的 <a href="/xmldom/dom_domexception.asp">DOMException 异常</a>：</p>

<p>INDEX_SIZE_ERR - 参数 <i>start</i> 或 <i>length</i> 是负数，或 <i>length</i> 大于 Text 节点或 Comment 节点的长度。</p>

<p>DOMSTRING_SIZE_ERR - 指定的文本范围太长，在浏览器的 JavaScript 实现中不能填到一个字符串中。</p>

<h3>描述</h3>

<p>该方法将从 Text 节点或 Comment 节点中提取从 <i>start</i> 开始的 <i>length</i> 个字符。只有当节点包含的文本的字符数大于浏览器的 JavaScript 实现中能填入的字符串的最大字符数，该方法才有用。在这种情况下，JavaScript 程序不能直接使用 Text 节点或 Comment 节点的 data 属性，而必须用节点文本的较短子串。在实际应用中，这种情况不太可能出现。</p>
</div>
...
<p>XML DOM 参考手册：<a href="/xmldom/met_text_replacedata.asp">Text.replaceData()</a></p>
<p>XML DOM 参考手册：<a href="/xmldom/met_comment_replacedata.asp">Comment.replaceData()</a></p>
</div>
