...
<title>XML DOM evaluate() 方法</title>
</head>
...
<h1>XML DOM evaluate() 方法</h1>

<div>
<p><a href="/xmldom/dom_document.asp">Document 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>evaluate() 方法计算一个 XPath 表达式。</p>

<h3>语法：</h3>
<pre>evaluate(<i>xpathText</i>,<i>contextNode</i>,<i>namespaceURLMapper</i>,<i>resultType</i>,<i>result</i>)</pre>
...
<tr>
<td>xpathText</td>
<td>表示要计算的 XPath 表达式的字符串。</td>
</tr>

<tr>
<td>contextNode</td>
<td>文档中，对应要计算的表达式的节点。</td>
</tr>

<tr>
<td>namespaceURLMapper</td>
<td>
<p>把一个命名空间前缀映射为一个全称命名空间 URL 的函数。</p>
<p>如果不需要这样的映射，就为 null。</p>
</td>
</tr>
...
<td>resultType</td>
<td>
<p>指定了期待作为结果的对象的类型，使用 XPath 转换来强制结果类型。</p>
<p>类型的可能的值是 XPathResult 对象所定义的常量。</p>
</td>
</tr>
...
<td>result</td>
<td>
<p>一个复用的 XPathResult 对象；</p>
<p>如果你要创建一个新的 XPathResult 对象，则为 null。</p>
</td>
</tr>
</table>

<h3>返回值</h3>
<p>表示根据给定的 Context 节点计算的表达式的一个 <a href="/xmldom/dom_xpathresult.asp" title="XML DOM XPathResult 对象">XPathResult 对象</a>。</p>

<h3>抛出</h3>
<p>如果 <i>xpathText</i> 包含语法错误，或者如果表达式的结果无法转换为想要的 <i>resultType</i>，或者如果表达式包含了 <i>namespaceURLMapper</i> 无法解析的命名空间，或者如果 <i>contextNode</i> 具有错误的类型或它和这个文档不相关，该方法都会抛出一个异常。</p>

<h3>描述</h3>

<p>该方法根据给定的 Context 节点来计算指定的 XPath 表达式，并且返回一个 XPathResult 对象，该对象是 type 来确定结果类型应该是什么。如果你想多次计算一个表达式，使用 <a href="/xmldom/met_document_createexpression.asp" title="XML DOM createExpression() 方法">Document.createExpression()</a> 来把表达式编译成一个 <a href="/xmldom/dom_xpathexpression.asp" title="XML DOM - XPathExpression 对象">XPathExpression 对象</a>，然后使用 XPathExpression 的 evaluate() 方法。</p>

<p>IE 不支持这个 API。参阅 <a href="/xmldom/met_node_selectnodes.asp" title="XML DOM selectNodes() 方法">Node.selectNodes()</a> 和 <a href="/xmldom/met_node_selectsinglenode.asp" title="XML DOM selectSingleNode() 方法">Node.selectSingleNode()</a> 来了解一个特定于 IE 的替代方法。</p>
</div>

<div>
<h2>参阅</h2>
<ul>
<li><a href="/xmldom/met_document_createexpression.asp" title="XML DOM createExpression() 方法">Document.createExpression()</a></li>
<li><a href="/xmldom/met_node_selectnodes.asp" title="XML DOM selectNodes() 方法">Node.selectNodes()</a></li>
<li><a href="/xmldom/met_node_selectsinglenode.asp" title="XML DOM selectSingleNode() 方法">Node.selectSingleNode()</a></li>
<li><a href="/xmldom/dom_xpathexpression.asp" title="XML DOM - XPathExpression 对象">XPathExpression</a></li>
<li><a href="/xmldom/dom_xpathresult.asp" title="XML DOM XPathResult 对象">XPathResult</a></li>
</ul>
</div>
