...
<title>XML DOM loadXML() 方法</title>
</head>
...
<div>
<h2>定义和用法</h2>
<p>loadXML() 方法通过解析一个 XML 标签字符串来组成该文档。</p>

<h3>语法：</h3>
<pre>loadXML(<i>text</i>)</pre>
...
<h3>说明</h3>
<p>这个特定于 IE 的方法解析指定的 XML 文本串，然后在当前文档对象中构建一棵 DOM 节点树，而丢弃此前存在于文档中的任何节点。</p>

<p>这个方法在表示 HTML 文档的 <a href="/xmldom/dom_document.asp" title="XML DOM - Document 对象">Document 对象</a>上并不存在。在调用 loadXML() 之前，通常创建一个新的、空的 <a href="/xmldom/dom_document.asp" title="XML DOM - Document 对象">Document 对象</a>来保存解析的内容：</p>

<pre>var doc = new ActivexObject (&quot;MSXML2.DOMDocument&quot;);
doc.loadXML(markup);</pre>

<p>参阅 <a href="/xmldom/dom_domparser.asp#parseFromString" title="DOMParser.parseFromString()">DOMParser.parseFromString()</a> 以了解一种非 IE 的替代方法。</p>
</div>

<div>
<h2>参阅</h2>
<p><a href="/xmldom/dom_domparser.asp#parseFromString" title="DOMParser.parseFromString()">DOMParser.parseFromString()</a></p>
</div>
