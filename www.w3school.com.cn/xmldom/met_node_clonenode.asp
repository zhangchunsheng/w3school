...
<title>XML DOM cloneNode() 方法</title>
</head>
...
<p>cloneNode() 方法可创建指定的节点的精确拷贝。</p>
<p>此方法可返回所复制的节点。</p>

<h3>语法：</h3>
<pre>nodeObject.cloneNode(include_all)</pre>
...
<tr>
<td>include_all</td>
<td>必需。假如逻辑参数被设置为真，那么被克隆的节点会克隆原节点的所有子节点。</td>
</tr>
</table>
...
<h3>说明</h3>
<p>该方法将复制并返回调用它的节点的副本。如果传递给它的参数是 true，它还将递归复制当前节点的所有子孙节点。否则，它只复制当前节点。</p>
<p>返回的节点不属于文档树，它的 parentNode 属性为 null。</p>
<p>当复制的是 Element 节点时，它的所有属性都将被复制。但要注意，当前节点上注册的事件监听器函数不会被复制。</p>
</div>
...
<p>下面的代码片段可克隆首个 &lt;book&gt; 节点，然后把它添加到节点列表的末尾：</p>
<pre>xmlDoc=loadXMLDoc(&quot;books.xml&quot;);

var x=xmlDoc.getElementsByTagName('book')[0];
var cloneNode=<code>x.cloneNode(true)</code>;
...
XQuery Kick Start
Learning XML
Everyday Italian</pre>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_clonenode">cloneNode() - 克隆节点</a></dt>
</dl>
</div>
