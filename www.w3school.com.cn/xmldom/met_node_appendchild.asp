...
<title>XML DOM appendChild() 方法</title>
</head>
...
<h1>XML DOM appendChild() 方法</h1>

<div>
<p><a href="/xmldom/dom_node.asp">Node 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>

<p>appendChild() 方法可向节点的子节点列表的末尾添加新的子节点。</p>
<p>此方法可返回这个新的子节点。</p>

<h3>语法：</h3>
<pre>appendChild(newchild)</pre>
...
<h3>描述</h3>
<p>该方法将把节点 newchild 添加到文档中，使它成为当前节点的最后一个子节点。</p>
<p>如果文档树中已经存在了 newchild，它将从文档树中删除，然后重新插入它的新位置。如果 newchild 是 DocumentFragment 节点，则不会直接插入它，而是把它的子节点按序插入当前节点的 childNodes[] 数组的末尾。</p>
<p>注意，来自一个文档的节点（或由一个文档创建的节点）不能插入另一个文档。也就是说，newchild 的 ownerDocument 属性必须与当前节点的 ownerDocument 属性相同。</p>

<h3>例子</h3>
<p>下面的函数将在文档末尾插入一个新段：</p>
<pre>
function appendMessage (message) {
  var pElement = document.createElement(&quot;p&quot;);
  var messageNode = document.createTextNode(message);
  <code>pElement.appendChild(messageNode);</code>
  <code>document.body.appendChild(pElement);</code>
}
</pre>
</div>
...
<p>下面的代码片段可创建并向首个 &lt;book&gt; 元素添加一个节点，然后输出首个 &lt;book&gt; 元素的所有子节点：</p>
...
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点间生成的空白文本节点（例如，换行符号），而 Mozilla 不会这样做。因此，在下面的例子中，我们仅处理元素节点（元素节点的 nodeType=1）。</p>
<p class="tip"><span>提示：</span>如需更多有关 IE 与 Mozilla 浏览器之间 XML DOM 的差异的内容，请访问我们的 <a href="dom_mozilla_vs_ie.asp" title="XML DOM 浏览器差异">DOM 浏览器</a> 章节。</p>
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_element_appendchild">appendChild() - 向首个 &lt;book&gt; 节点添加子节点</a></dt>
<dt><a href="/tiy/t.asp?f=xdom_createelement">appendChild() - 向所有 &lt;book&gt; 节点添加一个子节点</a></dt>
</dl>
</div>
