...
<title>XML DOM surroundContents() 方法</title>
</head>
...
<h1>XML DOM surroundContents() 方法</h1>
...
<div>
<h2>定义和用法</h2>
<p>surroundContents() 方法用指定的节点包围范围的内容。</p>

<h3>语法：</h3>
<pre>surroundContents(<i>newParent</i>)</pre>
...
<tr>
<td>newParent</td>
<td>将成为当前范围内容的新父节点的节点。</td>
</tr>
</table>


<h3>抛出</h3>
<p>该方法将在下列情况下抛出具有如下代码的 <a href="/xmldom/dom_domexception.asp" title="XML DOM - DOMException 对象">DOMException 异常</a>或 <a href="/xmldom/dom_rangeexception.asp" title="XML DOM - RangeException 对象">RangeException 异常</a>：</p>

<h4>DOMException.HIERARCHY_REQUEST_ERR</h4>
<p>当前范围的开始点的包容节点不能有子节点，不能有 <i>newParent</i> 类型的子节点，或者 <i>newParent</i> 是包容节点的祖先节点。</p>

<h4>DOMException.NO_MODIFICATION_ALLOWED_ERR</h4>
<p>当前范围的边界点的祖先节点是只读的，不允许进行插入操作。</p>

<h4>DOMException.WRONG_DOCUMENT_ERR</h4>
<p><i>newParent</i> 和它的范围是用不同 Document 对象创建的。</p>

<h4>RangeException.BAD_BOUNDARYPOINTS_ERR</h4>
<p>当前范围部分地选择了一个节点（除了 Text 节点外的），所以不能包围文档的这个区域。</p>

<h4>RangeException.INVALID_NODE_TYPE_ERR</h4>
<p><i>newParent</i> 是 Document、DocumentFragment、DocumentType、Attr、Entity 或 Notation 节点。</p>

<h3>描述</h3>
<p>该方法将把当前范围的父节点重定为 <i>newParent</i>，然后把 <i>newParent</i> 插在文档中范围的开始位置。例如，把文档的一个区域放入 &lt;div&gt; 或 &lt;span&gt; 元素中，可以使用该方法。</p>

<p>如果 <i>newParent</i> 已经是文档的一部分，那么它首先将从文档中删除，它的子节点也将被舍弃。当该方法返回时，该范围将以 <i>newParent</i> 之前的位置为开始点，<i>newParent</i> 之后的位置为结束点。</p>
</div>
