...
<title>XML DOM createDocumentType() 方法</title>
</head>
...
<h1>XML DOM createDocumentType() 方法</h1>
...
<p>createDocumentType() 方法创建一个 DocumentType 节点。</p>

<h3>语法：</h3>
<pre>nodeObject.createDocument(qualifiedName, publicId, systemId)</pre>
...
<td>
<p>文档类型的名字。</p>
<p>如果使用 XML 命名空间，该参数可能是一个限定名，用来指定命名空间前缀和本地名，两者之间用冒号分隔。</p>
</td>
</tr>

<tr>
<td>publicId</td>
<td>文档类型的公有标识符或 null。</td>
</tr>

<tr>
<td>systemId</td>
<td>
<p>文档类型的系统标识符或 null。</p>
<p>该参数通常声明了 DTD 文件的本地文件名。</p>
</td>
</tr>
</table>

<h3>返回值</h3>
<p>新创建的 DocumentType 对象，ownerdocument 属性为 null。</p>

<h3>说明</h3>
<p>该方法创建一个新的 DocumentType 节点。该方法只声明文档类型的一个外部子集。从 2 级 DOM 起，DOM 标准就不再提供声明内部子集的方法，返回的 DocumentType 对象不定义任何 Entity 节点和 Notation 节点。该方法只和 XML 文档一起使用，只支持 HTML 文档的实现不支持该方法。</p>
</div>
