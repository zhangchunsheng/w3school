...
<title>XML DOM insertRule() 方法</title>
</head>
...
<h1>XML DOM insertRule() 方法</h1>

<div class="backtoreference">
<p><a href="/xmldom/dom_cssstylesheet.asp" title="XML DOM CSSStyleSheet 对象">CSSStyleSheet 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>insertRule() 方法在样式表中插入一条规则。</p>

<h3>语法：</h3>
<pre>insertRule(<i>rule</i>,<i>index</i>)</pre>
...
<td>rule</td>
<td>
	<p>必需。要添加到样式表的规则的完整的、可解析的文本表示。</p>
	<ul class="listintable">
	<li>对于规则集（rule sets），<i>rule</i> 指示选择器和样式声明。</li>
	<li>对于 @ 规则（At rules），<i>rule</i> 指示 @ 标识符和规则内容。</li>
	</ul>
</td>
</tr>
...
<tr>
<td>index</td>
<td>必需。要把规则插入或附加到 cssRules 数组中的位置。</td>
</tr>
</table>
...
<p>该方法在下列情况下将抛出具有以下代码的 <a href="/xmldom/dom_domexception.asp">DOMException 异常</a>：</p>

<dl class="define">
<dt>HIERARCHY_REQUEST_ERR</dt>
<dd>CSS 语法不允许指定的规则出现在指定的位置。</dd>

<dt>INDEX_SIZE_ERR</dt>
<dd><i>index</i> 不对应样式表的规则集中的任何规则，比如 <i>index</i> 是负数或者大于 cssRules.length 的值。</dd>

<dt>NO_MODIFICATION_ALLOWED_ERR</dt>
<dd>该样式表是只读的。</dd>

<dt>SYNTAX</dt>
<dd>指定的 <i>rule</i> 文本具有文本错误。</dd>
</dl>

<h3>描述</h3>

<p>该方法将在样式表的 cssRules 数组的指定 <i>index</i> 处插入（或附加）新的 CSS rule。这是一个 DOM 标准方法，请参阅 <a href="/xmldom/met_cssstylesheet_addrule.asp">CSSStyleSheet.addRule()</a>，它是特定于 IE 的一个替代方法。</p>
...
<pre>myStyle.insertRule(&quot;#blanc { color: white }&quot;, 0);</pre>
...
<p>XML DOM 参考手册：<a href="/xmldom/met_cssstylesheet_addrule.asp">CSSStyleSheet.addRule()</a></p>
</div>
