...
<title>XML DOM nextSibling 属性</title>
</head>
...
<p>nextSibling 属性返回被选节点的下一个同级节点（在相同树层级中的下一个节点）。</p>

<p>如果不存在这样的节点，则该属性返回 NULL。</p>

<h3>语法：</h3>
<pre>elementNode.nextSibling</pre>
</div>

<div>
<h2>提示和注释</h2>
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点之间生成的空白文本节点（比如换行字符），而 Mozilla 不这么做。因此，在下面的例子中，我们用一个函数来检测下一个同级节点的节点类型。</p>
<p>元素节点的节点类型是 1，因此假如下一个同级节点不是元素节点，则移动到下一个节点，并检测该节点是否是元素节点。这个过程一直持续到找到下一个同级节点为止。这种方法可以确保在 Internet Explorer 和 Mozilla 都获得正确的结果。</p>
...
<p>下面的代码片段获取 XML 文档中的一个 &lt;title&gt; 元素的下一个同级节点：</p>

<pre>//check if the next sibling node is an element node
function get_nextsibling(n)
{
var x=n.nextSibling;
while (x.nodeType!=1)
 {
 x=<code>x.nextSibling</code>;
 }
return x;
}
...
Next sibling: author = Giada De Laurentiis</pre>
</div>
