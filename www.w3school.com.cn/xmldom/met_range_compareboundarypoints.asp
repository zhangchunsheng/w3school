...
<title>XML DOM compareBoundaryPoints() 方法</title>
</head>
...
<h1>XML DOM compareBoundaryPoints() 方法</h1>
...
<div>
<h2>定义和用法</h2>
<p>compareBoundaryPoints() 方法比较两个范围的位置。</p>

<h3>语法：</h3>
<pre>compareBoundaryPoints(<i>how</i>,<i>sourceRange</i>)</pre>
...
<tr>
<td>how</td>
<td>声明如何执行比较操作（即比较哪些边界点）。它的合法值是 Range 接口定义的常量。</td>
</tr>
...
<h3>返回值</h3>
<p>如果当前范围的指定边界点位于 <i>sourceRange</i> 指定的边界点之前，则返回 -1。如果指定的两个边界点相同，则返回 0。如果当前范围的边界点位于 <i>sourceRange</i> 指定的边界点之后，则返回 1。</p>

<h3>抛出</h3>
<p>如果 <i>sourceRange</i> 表示的文档不同于当前范围表示的文档，该方法将抛出代码为 WRONG_DOCUMENT_ERR 的 <a href="/xmldom/dom_domexception.asp" title="XML DOM - DOMException 对象">DOMException 异常</a>。</p>

<h3>描述</h3>
<p>该方法将比较当前范围的边界点和指定的 <i>sourceRange</i> 的边界点，并返回一个值，声明它们在源文档中的相对位置。参数 <i>how</i> 指定了比较两个范围的哪个边界点。该参数的合法值和它们的含义如下：</p>

<ul>
<li>Range.START_TO_START - 比较两个 Range 节点的开始点</li>
<li>Range.END_TO_END - 比较两个 Range 节点的结束点</li>
<li>Range.START_TO_END - 用 <i>sourceRange</i> 的开始点与当前范围的结束点比较</li>
<li>Range.END_TO_START - 用 <i>sourceRange</i> 的结束点与当前范围的开始点比较</li>
</ul>

<p>该方法的返回值是一个数字，声明了当前范围相对于 <i>sourceRange</i> 的位置。因此，您可能认为，首先需要用参数 <i>how</i> 的范围常量指定当前范围的边界点，然后再用它指定 <i>sourceRange</i> 的边界点。但事实上，常量 Range.START_TO_END 指定与当前范围的 end 点和 <i>sourceRange</i> 的 start 点进行比较。同样，常量 Range.END_TO_START 指定比较当前范围的 start 点和指定范围的 end 点。</p>
</div>
