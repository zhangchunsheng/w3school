...
<title>XML DOM importNode() 方法</title>
</head>
...
<div>
<h2>定义和用法</h2>
<p>importNode() 方法把一个节点从另一个文档复制到该文档以便应用。</p>

<h3>语法：</h3>
<pre>importNode(<i>importedNode</i>,<i>deep</i>)</pre>
...
<tr>
<td>deep</td>
<td>如果为 true，还要递归复制 importedNode 节点的所有子孙节点。</td>
</tr>
</table>

<h3>返回值</h3>
<p><i>importedNode</i> （可能还有它的子孙节点）的副本，它的 ownerDocument 属性设置到该文档。</p>

<h3>抛出</h3>
<p>如果 <i>importedNode</i> 是 Document 节点或 DocumentType 节点，该方法将抛出代码为 NOT_SUPPORTED_ERR 的 <a href="/xmldom/dom_domexception.asp" title="XML DOM - DOMException 对象">DOMException 异常</a>，因为不能导入这些类型的节点。</p>

<h3>说明</h3>
<p>该方法的参数是另一个文档中定义的节点，返回值是适合插入该文档的节点的副本。如果 <i>deep</i> 值为 true，那么还要复制该节点的所有子孙节点。无论如何，原始节点和它的子孙节点都不会被修改。</p>

<p>返回的副本的 ownerDocument 属性被设置为当前文档，但是 parentNode 属性为 null，因为它还没有插入文档。在原始节点或树中注册的事件监听器函数不会被复制。</p>

<p>当导入 Element 节点时，只有在源文档中明确设置的属性才会被导入。当导入 Attr 节点时，将自动把它的 specified 属性设置为 true。</p>
</div>

<div>
<h2>参阅</h2>
<p><a href="/xmldom/met_node_clonenode.asp" title="XML DOM cloneNode() 方法">Node.cloneNode()</a></p>
</div>
