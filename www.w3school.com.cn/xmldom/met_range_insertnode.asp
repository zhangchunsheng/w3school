...
<title>XML DOM insertNode() 方法</title>
</head>
...
<h1>XML DOM insertNode() 方法</h1>

<div class="backtoreference">
<p><a href="/xmldom/dom_range.asp" title="XML DOM - Range 对象">Range 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>insertNode() 方法在范围的开头插入一个节点。</p>

<h3>语法：</h3>
<pre>insertNode(<i>newNode</i>)</pre>
...
<h3>抛出</h3>
<p>如果 <i>newNode</i> 是 Attr、Document、Entity 或 Notation 节点，该方法将抛出代码为 INVALID_NODE_TYPE_ERR 的 <a href="/xmldom/dom_rangeexception.asp" title="XML DOM - RangeException 对象">RangeException 异常</a>。</p>
<p>在下列条件下，该方法还将抛出如下代码的 DOMException 异常：</p>

<ul>
<li>HIERARCHY_REQUEST_ERR - 包含范围的开始点的节点不能有子节点，它也不能有指定类型的子节点，或者 <i>newNode</i> 是该节点的祖先节点（或是该节点自身）。</li>
<li>NO_MODIFICATION_ALLOWED_ERR - 包含范围的开始点的节点（或它的祖先节点）是只读的。</li>
<li>WRONG_DOCUMENT_ERR - <i>newNode</i> 与范围所属于的文档不同。</li>
</ul>

<h3>描述</h3>
<p>该方法将把指定的节点（和它的所有子孙节点）插入文档范围的开始点。当该方法返回时，当前范围将包括新插入的节点。如果 <i>newNode</i> 已经是文档的一部分，那么它将被从当前位置删除，然后重新插入范围的开始点。如果 <i>newNode</i> 是 <a href="/xmldom/dom_documentfragment.asp" title="XML DOM - DocumentFragment 对象">DocumentFragment 节点</a>，那么插入的不是它自身，而是它的子孙节点，按顺序插入范围的开始点。</p>
<p>如果包含当前范围的开始点的节点是 Text 节点，那么在发生插入操作前，它将被分割成两个相邻的节点。如果 <i>newNode</i> 是 Text 节点，在插入文档后，它不会与任何相邻的 Text 节点合并。要合并相邻的节点，需要调用 <a href="/xmldom/met_node_normalize.asp" title="XML DOM normalize() 方法">Node.normalize()</a> 方法。</p>

<h3>参阅</h3>
<p>
<a href="/xmldom/dom_documentfragment.asp" title="XML DOM - DocumentFragment 对象">DocumentFragment 对象</a>、
<a href="/xmldom/met_node_normalize.asp" title="XML DOM normalize() 方法">Node.normalize()</a>。
</p>
</div>
