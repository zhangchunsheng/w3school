...
<p>getElementsByTagName() 方法返回拥有指定名称的所有元素的 NodeList。</p>

<h3>语法：</h3>
<pre>getElementsByTagName(name)</pre>
...
<tr>
<td>name</td>
<td>字符串值，规定要搜索的标签名。值 &quot;*&quot; 匹配所有的节点。</td>
</tr>
</table>

<h3>说明</h3>
<p>该方法将遍历指定元素的子孙节点，返回一个 Element 节点的数组（实际上是 NodeList 对象），表示所有具有指定标签名的文档元素。元素在返回的数组中的顺序就是它们出现在文档源代码中的顺序。</p>
<p>注意，Document 接口也定义了 getElementsByTagName() 方法，它与该方法相似，但遍历整个文档，而不是遍历某个元素的子孙节点。</p>
</div>
...
<p>下面的代码片段显示 &quot;books.xml&quot; 中所有 &lt;title&gt; 元素的值：</p>

<pre>xmlDoc=loadXMLDoc(&quot;books.xml&quot;);

x=<code>xmlDoc.getElementsByTagName('title')</code>;
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_getelementsbytagname">getElementsByTagName() - 通过名称来获取指定元素的值</a></dt>
</dl>
</div>
