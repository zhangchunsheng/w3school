...
<title>XML DOM splitText() 方法</title>
</head>
...
<h1>XML DOM splitText() 方法</h1>

<div class="backtoreference">
<p><a href="/xmldom/dom_text.asp" title="XML DOM - Text 对象">Text 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>splitText() 方法按照指定的 offset 把文本节点分割为两个节点。</p>

<h3>语法：</h3>
<pre>replaceData(offset)</pre>
...
<tr>
<td>offset</td>
<td>必需。规定在何处分割文本节点。开始值以 0 开始。</td>
</tr>
</table>

<h3>返回值</h3>
<p>从当前节点分割出的 Text 节点。</p>

<h3>说明</h3>
<p>该方法将在指定的 offset 处把 Text 节点分割成两个节点。原始的 Text 节点将被修改，使它包含 offset 指定的位置之前的文本内容（但不包括文本内容）。新的 Text 节点将被创建，用于存放从 offset 位置（包括该位置上的字符）到原字符结尾的所有字符。新的 Text 节点是该方法的返回值。此外，如果原始的 Text 节点具有 parentNode，新的 Text 节点将插入这个父节点，紧邻在原始节点之后。</p>

<p><a href="/xmldom/dom_cdatasection.asp" title="XML DOM CDATASection 对象">CDATASection 接口</a>继承了 Text 接口，CDATASection 节点也可以使用该方法 ，只是新创建的节点是 CDATASection 节点，而不是 Text 节点。</p>
</div>
...
<p>下面的代码片段将在第一个单词之后分割 Text 节点：</p>

<pre>xmlDoc=loadXMLDoc(&quot;books.xml&quot;);

x=xmlDoc.getElementsByTagName(&quot;title&quot;)[0].childNodes[0];
<code>y=x.splitText(9);</code>
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_text_splittext">replaceData() - 替换文本节点中的数据</a></dt>
</dl>
</div>

<div>
<h2>参阅</h2>
<p><a href="met_node_normalize.asp" title="XML DOM normalize() 方法">Node.normalize()</a></p>
</div>
