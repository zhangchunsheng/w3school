...
<title>XML DOM createEvent() 方法</title>
</head>
...
<div>
<h2>定义和用法</h2>
<p>createEvent() 方法创建新的 <a href="/xmldom/dom_event.asp" title="XML DOM Event 对象">Event 对象</a>。</p>

<h3>语法：</h3>
<pre>createEvent(<i>eventType</i>)</pre>
...
<tr>
<td>eventType</td>
<td><p>想获取的 Event 对象的事件模块名。</p>
<p>关于有效的事件类型列表，请参阅“<a href="#eventType">说明</a>”部分。</p></td>
</tr>
</table>

<h3>返回值</h3>
<p>返回新创建的 <a href="/xmldom/dom_event.asp" title="XML DOM Event 对象">Event 对象</a>，具有指定的类型。</p>

<h3>抛出</h3>
<p>如果实现支持需要的事件类型，该方法将抛出代码为 NOT_SUPPORTED_ERR 的 <a href="/xmldom/dom_domexception.asp" title="XML DOM - DOMException 对象">DOMException 异常</a>。</p>


<h3><a id="eventType">说明</a></h3>

<p>该方法将创建一种新的事件类型，该类型由参数 <i>eventType</i> 指定。注意，该参数的值不是要创建的事件接口的名称，而是定义那个接口的 DOM 模块的名称。</p>
<p>下表列出了 <i>eventType</i> 的合法值和每个值创建的事件接口：</p>
...
<p>用该方法创建了 Event 对象以后，必须用上表中所示的初始化方法初始化对象。关于初始化方法的详细信息，请参阅 <a href="/xmldom/dom_event.asp" title="XML DOM Event 对象">Event 对象参考</a>。</p>
<p>该方法实际上不是由 Document 接口定义的，而是由 DocumentEvent 接口定义的。如果一个实现支持 Event 模块，那么 <a href="/xmldom/dom_document.asp" title="XML DOM - Document 对象">Document 对象</a>就会实现 DocumentEvent 接口并支持该方法。</p>
</div>
