...
<title>XML DOM iterateNext() 方法</title>
</head>
...
<h1>XML DOM iterateNext() 方法</h1>

<div class="backtoreference">
<p><a href="/xmldom/dom_xpathresult.asp" title="XML DOM XPathResult 对象">XPathResult 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>iterateNext() 方法返回和一个 XPath 查询匹配的下一个节点。</p>

<h3>语法：</h3>
<pre>iterateNext()</pre>

<h3>返回值</h3>
<p>返回匹配节点列表中的下一个节点，如果没有其他节点，则为 null。</p>

<h3>抛出</h3>
<p>既然 XPathResult 被返回，如果文档已经修改了，这个方法会抛出异常。如果 resultType 不是 UNORDERED_NODE_ITERATOR_TYPE 或 ORDERED_NODE_ITERATOR_TYPE 的时候调用，方法也会抛出异常。</p>

<h3>说明</h3>
<p>iterateNext() 方法返回与 XPath 查询匹配的下一个节点，如果所有匹配节点都已经返回了，则为 null。</p>

<p>当 XPathResult 是 UNORDERED_NODE_ITERATOR_TYPE 或 ORDERED_NODE_ITERATOR_TYPE 的时候使用这个方法。如果类型是有序的，节点按照它们在文档中出现的顺序返回，否则，它们按照任意顺序返回。</p>

<p>如果 invalidIteratorState 属性为 true，文档已经被修改了，这个方法抛出异常。</p>
</div>
