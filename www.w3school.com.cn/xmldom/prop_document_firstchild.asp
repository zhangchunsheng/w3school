...
<title>XML DOM firstChild 属性</title>
</head>
...
<div>
<h2>定义和用法</h2>
<p>firstChild 属性可返回文档的首个子节点。</p>

<h3>语法：</h3>
<pre>documentObject.firstChild </pre>
</div>

<div>
<h2>提示和注释</h2>
<p class="note"><span>注释：</span>Internet Explorer 会忽略节点间生成的空白文本节点（例如，换行符号），而 Mozilla 不会这样做。因此，在下面的例子中，我们会使用一个函数来检查第一个子节点的节点类型。</p>
<p>元素节点的节点类型是 1，因此如果首个子节点不是一个元素节点，它就会移至下一个节点，然后继续检查此节点是否为元素节点。整个过程会一直持续到首个元素子节点被找到为止。通过这个方法，我们就可以在 Internet Explorer 和 Mozilla 得到正确的结果。</p>
...
<p>以下代码片段可显示文档首个子节点的节点名称和节点类型：</p>
<pre>
<span>//检查第一个节点是否是 element 节点</span>
function get_firstchild(n)
{
var x=<code>n.firstChild</code>;
while (x.nodeType!=1)
{
x=x.nextSibling;
}
...
<h3>输出：</h3>
<pre>Nodename: bookstore (nodetype: 1)</pre>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_doc_firstchild">取得文档的第一个子节点</a></dt>
<dt><a href="/tiy/t.asp?f=xdom_doc_lastchild">取得文档的最后一个子节点</a></dt>
</dl>
</div>
