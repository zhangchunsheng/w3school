...
<p><a href="/xmldom/dom_comment.asp" title="XML DOM - Comment 对象">Comment 对象参考手册</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>substringData() 方法从注释节点中提取子串。</p>

<h3>语法：</h3>
<pre>commentNode.substringData(<i>start</i>,<i>length</i>)</pre>
...
<tr>
<td>start</td>
<td>必需。规定要返回的第一个字符的位置。该值从 0 开始。</td>
</tr>

<tr>
<td>length</td>
<td>必需。规定要返回的子串中的字符数。</td>
</tr>
</table>

<h3>返回值</h3>

<p>返回一个字符串，包含 Comment 节点中从 <i>start</i> 开始的 <i>length</i> 个字符。</p>

<h3>描述</h3>

<p>该方法从 Comment 节点返回从 <i>start</i> 开始的 <i>length</i> 个字符。只有当节点包含的文本的字符数大于浏览器的 JavaScript 实现中能填入的字符串的最大字符数，该方法才有用。在这种情况下，JavaScript 程序不能直接使用 Comment 节点的 data 属性，而必须用节点文本的较短子串。在实际应用中，这种情况不太可能出现。</p>
</div>
...
<p>以下代码段使用 JavaScript 函数 <a href="/xmldom/dom_loadxmldoc.asp" title="XML DOM 加载函数">loadXMLDoc()</a> 把 XML 文件 <a href="/example/xdom/books_comment.xml">books_comment.xml</a> 载入 xmlDoc 中，然后从第一个注释节点中返回子串（&quot;Hardcover&quot;）：</p>
...
if (x[i].nodeType==8)
  ...
  <code>y=x[i].substringData(10,9);</code>
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=xdom_comment_substringData">substringData() - 从注释节点返回子串</a></dt>
</dl>
</div>
...
<p>XML DOM 参考手册：<a href="/xmldom/met_characterdata_substringdata.asp">CharacterData.substringData()</a></p>
</div>
