...
<title>XML DOM dispatchEvent() 方法</title>
</head>
...
<h1>XML DOM dispatchEvent() 方法</h1>
...
<p>dispatchEvent() 方法给节点分派一个合成事件。</p>


<h3>语法：</h3>
<pre>dispatchEvent(<i>evt</i>)</pre>
...
<tr>
<td>evt</td>
<td>必需。要分派的 Event 对象。</td>
</tr>
</table>


<h3>返回值</h3>
<p>如果在事件传播过程中调用了 <i>evt</i> 的 <a href="/xmldom/met_event_preventdefault.asp" title="XML DOM preventDefault() 方法">preventDefault() 方法</a>，则返回 false，否则返回 true。</p>


<h3>抛出</h3>
<p>如果 Event 对象 <i>evt</i> 没有被初始化，或者它的 type 属性为 null 或空串，该方法将抛出异常。</p>

<h3>描述</h3>
<p>该方法将分派一个合成事件，它由 <a href="/xmldom/met_document_createevent.asp" title="XML DOM createEvent() 方法">Document.createEvent()</a> 创建，由 Event 接口或它的某个子接口定义的初始化方法初始化。</p>

<p>调用该方法的节点将成为事件的目标节点，该事件在捕捉阶段中第一次沿着文档树向下传播。如果该事件的 bubbles 属性为 true，那么在事件的目标节点自身处理事件后，它将沿着文档树向上起泡。</p>
</div>
