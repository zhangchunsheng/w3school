...
<title>ASP.NET Web Pages - 文件夹</title>
...
<li class="currentLink"><a href="/aspnet/webpages_folders.asp" title="WebPages 文件夹">WebPages 文件夹</a></li>
...
<h1>ASP.NET Web Pages - 文件夹</h1>


<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/aspnet/webpages_layout.asp" title="WebPages 布局">WebPages 布局</a></li>
<li class="next"><a href="/aspnet/webpages_global.asp" title="WebPages 全局">WebPages 全局</a></li>
</ul>
</div>
...
<p>下面是 ASP.NET 网页的典型文件夹结构：</p>

<img src="/i/webpages_folders.jpg" alt="Web Pages 文件夹结构" />
<ul>
<li>&quot;Account&quot; 文件夹包含登录和安全文件</li>
<li>&quot;App_Data&quot; 文件包含数据库和数据文件</li>
<li>&quot;Images&quot; 文件夹包含图片</li>
<li>&quot;Scripts&quot; 文件夹包含浏览器脚本</li>
<li>&quot;Shared&quot; 文件夹包含通用文件（比如布局和样式文件）</li>
</ul>
</div>
...
<p>上面这个网站的 &quot;Images&quot; 文件夹的物理结构在计算机上类似这样：</p>

<pre>C:\Johnny\Documents\MyWebSites\Demo\Images</pre>
</div>
...
<p>web 图片的虚拟名称类似这样：&quot;Images/pic31.jpg&quot;。</p>

<p>但是物理名称类似这样 &quot;C:\Johnny\Documents\MyWebSites\Demo\Images\pic31.jpg&quot;。</p>
</div>
...
<p>URL 用于从 web 访问文件：</p>

<pre>http://www.w3school.com.cn/html5/html5_intro.asp</pre>

<p>URL 在服务器上对应的文件是：</p>

<pre>C:\MyWebSites\htm5\html5_intro.asp</pre>

<p>虚拟路径是代表物理路径的简写形式。如果您使用虚拟路径，就能够在不更新路径的情况下将网页移动到不同的域名（或服务器）。</p>
...
<p>磁盘驱动器上的根目录写作： C:\，但是网站上的根目录是  /（斜杠）。</p>

<p>web 文件夹的虚拟路径（几乎）永远不会与物理文件夹相同。</p>

<p>在您的代码中，根据代码内容的不同，可能会同时引用物理路径和虚拟路径。</p>

<p>ASP.NET 提供三种处理文件夹路径的工具：~ 操作符、Server.MapPath 方法以及 Href 方法。</p>
</div>
...
<p>如需在编程代码中规定虚拟根目录，请使用 ~ 操作符。</p>

<p>如果您使用 ~ 操作符，而不是路径，就能够在不改变任何代码的情况下，将网站移动到不同的文件夹或位置：</p>
...
<div>
<h2>Server.MapPath 方法</h2>

<p>Server.MapPath 方法将虚拟路径 (/default.cshtml) 转换为服务器能够理解的物理路径 (C:\Johnny\MyWebSited\Demo\default.cshtml)。</p>

<p>当需要打开位于服务器上的数据文件时，您就会用到该方法（数据文件只能通过完整的物理路径来访问）：</p>
...
<p>在本教程的下一章中，您将学习更多有关在服务器上读取和写入数据文件的知识。</p>
</div>
...
<p>Href 方法把代码中的路径转换为浏览器能够理解的路径（浏览器无法理解 ~ 操作符）。</p>

<p>您使用 Href 方法来创建指向资源（比如图片和 CSS 文件）的路径。</p>

<p>您会经常在 HTML &lt;a&gt;、&lt;img&gt; 以及 &lt;link&gt; 元素中用到该方法：</p>

<pre>
@{var myStyleSheet = &quot;~/Shared/Site.css&quot;;}

<span class="code_comment">&lt;!-- 创建指向 CSS 文件的链接 --&gt;</span>
&lt;link rel=&quot;stylesheet&quot; type=&quot;text/css&quot; href=&quot;@Href(myStyleSheet)&quot; /&gt;

<span class="code_comment">&lt;!-- 同上 : --&gt;</span>
&lt;link rel=&quot;stylesheet&quot; type=&quot;text/css&quot; href=&quot;/Shared/Site.css&quot; /&gt;
</pre>

<p>Href 方法属于 WebPage 对象的方法。</p>
</div>
