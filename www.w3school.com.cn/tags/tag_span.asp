...
<li class="currentLink"><a href="/tags/tag_span.asp" title="HTML &lt;span&gt; 标签">&lt;span&gt;</a></li>
...
<p class="tip"><span>提示：</span>请使用 &lt;span&gt; 来组合行内元素，以便通过样式来格式化它们。</p>

<p class="note"><span>注释：</span>span 没有固定的格式表现。当对它应用样式时，它才会产生视觉上的变化。</p>
</div>
...
<p>可以对同一个 &lt;span&gt; 元素应用 class 或 id 属性，但是更常见的情况是只应用其中一种。这两者的主要差异是，class 用于元素组（类似的元素，或者可以理解为某一类元素），而 id 用于标识单独的唯一的元素。</p>

<p class="tip"><span>提示：</span>事实上，您也许已经注意到了，W3School 站点上有一些文本的样式与其他文本是不同的。比如“提示”使用了粗体的橘红色。尽管实现这种效果的方法非常多，但是我们的做法是：使用“提示”使用 span 元素，然后对这个 span 元素的父元素，即 p 元素应用 class，这样就可以对这个类的子元素 span 应用相应的样式了。</p>

<p>HTML:</p>

<pre>&lt;p class=&quot;tip&quot;&gt;&lt;span&gt;提示：&lt;/span&gt;... ... ...&lt;/p&gt;</pre>
...
<p>&lt;span&gt; 标签支持 <a href="/tags/html_ref_standardattributes.asp">HTML 中的全局属性</a>。</p>
</div>
...
<p>&lt;span&gt; 标签支持 <a href="/tags/html_ref_eventattributes.asp">HTML 中的事件属性</a>。</p>
</div>
