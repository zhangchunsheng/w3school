...
<li class="currentLink"><a href="/tags/tag_canvas.asp" title="HTML &lt;canvas&gt; 标签">&lt;canvas&gt;</a></li>
...
<h1>HTML &lt;canvas&gt; 标签</h1>


<div>
<h2>实例</h2>

<p>如何通过 canvas 元素来显示一个红色的矩形：</p>
...
<p class="tiy"><a target="_blank" href="/tiy/t.asp?f=html5_canvas">亲自试一试</a></p>
</div>
...
<p>Internet Explorer 9+, Firefox, Opera, Chrome 以及 Safari 支持 &lt;canvas&gt; 标签。</p>

<p class="note"><span>注释：</span>Internet Explorer 8 以及更早的版本不支持 &lt;canvas&gt; 标签。</p>
</div>
...
<table class="dataintable"> 
  ...
  <tr>
    <td class="html5_new"><a href="/tags/att_canvas_height.asp" title="HTML5 &lt;canvas&gt; height 属性">height</a></td>
    <td><i>pixels</i></td>
    <td>设置 canvas 的高度。</td>
  </tr>
  <tr>
    <td class="html5_new"><a href="/tags/att_canvas_width.asp" title="HTML5 &lt;canvas&gt; width 属性">width</a></td>
    <td><i>pixels</i></td>
    <td>设置 canvas 的宽度。</td>
  </tr>
</table>
...
<p>&lt;canvas&gt; 标签支持 <a href="/tags/html_ref_standardattributes.asp">HTML 中的全局属性</a>。</p>
</div>
...
<p>&lt;canvas&gt; 标签支持 <a href="/tags/html_ref_eventattributes.asp">HTML 中的事件属性</a>。</p>
</div>
...
<p>这个 HTML 元素是为了客户端矢量图形而设计的。它自己没有行为，但却把一个绘图 API 展现给客户端 JavaScript 以使脚本能够把想绘制的东西都绘制到一块画布上。</p>

<p>&lt;canvas&gt; 标记由 Apple 在 Safari 1.3 Web 浏览器中引入。对 HTML 的这一根本扩展的原因在于，HTML 在 Safari 中的绘图能力也为 Mac OS X 桌面的 Dashboard 组件所使用，并且 Apple 希望有一种方式在 Dashboard 中支持脚本化的图形。</p>

<p>Firefox 1.5 和 Opera 9 都跟随了 Safari 的引领。这两个浏览器都支持 &lt;canvas&gt; 标记。</p>

<p>我们甚至可以在 IE 中使用 &lt;canvas&gt; 标记，并在 IE 的 VML 支持的基础上用开源的 JavaScript 代码（由 Google 发起）来构建兼容性的画布。
参见：<a href="http://excanvas.sourceforge.net/" title="ExplorerCanvas">http://excanvas.sourceforge.net/</a>。</p>

<p>&lt;canvas&gt; 的标准化的努力由一个 Web 浏览器厂商的非正式协会在推进，目前 &lt;canvas&gt; 已经成为 HTML 5 草案中一个正式的标签。
参见：<a href="http://www.whatwg.org/specs/web-apps/current-work/" title="HTML 5">http://www.whatwg.org/specs/web-apps/current-work/</a></p>
</div>
...
<p>这两种方式在功能上是等同的，任何一种都可以用另一种来模拟。从表面上看，它们很不相同，可是，每一种都有强项和弱点。例如，SVG 绘图很容易编辑，只要从其描述中移除元素就行。</p>

<p>要从同一图形的一个 &lt;canvas&gt; 标记中移除元素，往往需要擦掉绘图重新绘制它。</p>
</div>
...
<p>大多数 Canvas 绘图 API 都没有定义在 &lt;canvas&gt; 元素本身上，而是定义在通过画布的 <a href="/jsref/met_canvas_getcontext.asp" title="HTML DOM getContext() 方法">getContext() 方法</a>获得的一个“绘图环境”对象上。</p>
...
<p class="note"><span>注释：</span>Canvas API 非常紧凑的一个原因上它没有对绘制文本提供任何支持。要把文本加入到一个 &lt;canvas&gt; 图形，必须要么自己绘制它再用位图图像合并它，或者在 &lt;canvas&gt; 上方使用 CSS 定位来覆盖 HTML 文本。</p>
</div>
