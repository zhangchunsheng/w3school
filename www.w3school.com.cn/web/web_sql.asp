...
<li class="currentLink"><a href="/web/web_sql.asp" title="SQL 初级教程">SQL</a></li>
</ul>
</div><div id="selected">
<h2>建站手册</h2>
<ul>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/web/web_scripting.asp" title="服务器脚本 初级教程">服务器脚本</a></li>
<li class="next"><a href="/web/index.asp" title="网站构建初级教程">网站构建初级教程</a></li>
</ul>
</div>

<div id="intro">
<p><strong>SQL 是用于访问和处理数据库的标准的计算机语言。</strong></p>
</div>

<div>
<h2>通过 SQL 来管理数据</h2>

<p>结构化查询语言 (SQL) 是用于访问数据库的标准语言，这些数据库包括 SQL Server、Oracle、MySQL、Sybase 以及 Access 等等。</p>

<p>对于那些希望在数据库中存储数据并从中获取数据的人来说，SQL 的知识是价值无法衡量的。</p>
</div>

<div>
<h2>什么是 SQL?</h2>
<ul>
<li>SQL 指结构化查询语言 (<em>S</em>tructured <em>Q</em>uery <em>L</em>anguage)</li>
...
<p class="note"><span>编者注：</span>ANSI，美国国家标准化组织</p>
</div>
...
<p class="note"><span>注释：</span>除了 SQL 标准之外，大部分 SQL 数据库程序都拥有它们自己的私有扩展！</p>
</div>
...
<p>上面的表包含三条记录（每一条对应一个人）和四个列（姓、名、地址和城市）。</p>
</div>

<div>
<h2>SQL 查询程序</h2>
<p>通过 SQL，我们可以查询某个数据库，并获得返回的一个结果集。</p>
<p>查询程序类似这样：</p>
...
<tr><td>Hansen</td></tr>
<tr><td>Svendson</td></tr>
<tr><td>Pettersen</td></tr>
</table>

<p class="note"><span>注释：</span>某些数据库系统要求在 SQL 命令的末端使用分号。在我们的教程中不使用分号。</p>
</div>

<div>
<h2>SQL 数据操作语言 (DML)</h2>
<p>SQL (结构化查询语言)是用于执行查询的语法。但是 SQL 语言也包含用于更新、插入和删除记录的语法。</p>
<p>这些查询和更新语句都来自 SQL 的 DML 部分：</p>
<ul>
...
<div>
<h2>SQL 数据定义语言 (DDL)</h2>
<p>SQL 的数据定义语言部分使我们有能力创建或删除表格。我们也可以定义索引（键），规定表之间的链接，以及施加表间的约束。</p>
...
<div>
<h2>SQL 教程</h2>
<p>学习 W3School 提供的 <a href="/sql/index.asp" title="SQL 教程">SQL 教程</a>。</p>
</div>
