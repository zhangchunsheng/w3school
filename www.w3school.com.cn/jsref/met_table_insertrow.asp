...
<div>
<h2>定义和用法</h2>
<p>insertRow() 方法用于在表格中的指定位置插入一个新行。</p>

<h3>语法</h3>
<pre>tableObject.insertRow(index)</pre>

<h3>返回值</h3>
<p>返回一个 TableRow，表示新插入的行。</p>

<h3>说明</h3>
<p>该方法创建一个新的 TableRow 对象，表示一个新的 &lt;tr&gt; 标记，并把它插入表中的指定位置。</p>
<p>新行将被插入 index 所在行之前。若 index 等于表中的行数，则新行将被附加到表的末尾。</p>
<p>如果表是空的，则新行将被插入到一个新的 &lt;tbody&gt; 段，该段自身会被插入表中。</p>

<h3>抛出</h3>
<p>若参数 index 小于 0 或大于等于表中的行数，该方法将抛出代码为 INDEX_SIZE_ERR 的 <a href="/xmldom/dom_domexception.asp" title="XML DOM - DOMException 对象">DOMException 异常</a>。</p>
</div>

<div>
<h2>提示和注释</h2>
<p class="tip"><span>提示：</span>可以用 TableRow.insertCell() 方法给新创建的行添加内容。</p>
</div>

<div>
<h2>实例</h2>
<p>下面的例子在表格的开头插入一个新行：</p>
<pre>&lt;html&gt;
&lt;head&gt;
&lt;script type=&quot;text/javascript&quot;&gt;
function insRow()
  {
  <code>document.getElementById('myTable').insertRow(0)</code>
  }
&lt;/script&gt;
&lt;/head&gt;
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=hdom_table_insertrow">向表格添加新行 - 然后向其添加内容</a></dt>
</dl>
</div>
