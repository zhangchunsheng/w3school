...
<title>HTML DOM insertCell() 方法</title>
...
<h1>HTML DOM insertCell() 方法</h1>

<div class="backtoreference">
<p><a href="/jsref/dom_obj_tablerow.asp">HTML DOM Tablerow 对象</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>insertCell() 方法用于在 HTML 表的一行的指定位置插入一个空的 &lt;td&gt; 元素。</p>

<h3>语法</h3>
<pre>tablerowObject.insertCell(index)</pre>

<h3>返回值</h3>
<p>一个 TableCell 对象，表示新创建并被插入的 &lt;td&gt; 元素。</p>

<h3>说明</h3>
<p>该方法将创建一个新的 &lt;td&gt; 元素，把它插入行中指定的位置。新单元格将被插入当前位于 index 指定位置的表元之前。如果 index 等于行中的单元格数，则新单元格被附加在行的末尾。</p>

<p>请注意，该方法只能插入 &lt;td&gt; 数据表元。若需要给行添加头表元，必须用 Document.createElement() 方法和 Node.insertBefore() 方法（或相关的方法）创建并插入一个 &lt;th&gt; 元素。</p>

<h3>抛出</h3>
<p>若参数 index 小于 0 或大于等于行中的的表元数，该方法将抛出代码为 INDEX_SIZE_ERR 的 <a href="/xmldom/dom_domexception.asp" title="XML DOM - DOMException 对象">DOMException 异常</a>。</p>
</div>

<div>
<h2>实例</h2>
<p>下面的例子在表格行中插入了一个单元格：</p>
<pre>&lt;html&gt;
&lt;head&gt;
&lt;script type=&quot;text/javascript&quot;&gt;
function insCell()
  {
  var x=<code>document.getElementById('tr2').insertCell(0)</code>
  x.innerHTML=&quot;John&quot;
  }
&lt;/script&gt;
&lt;/head&gt;
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=hdom_tablerow_insertcell">向一个已有的行中插入单元格</a></dt>
<dt><a href="/tiy/t.asp?f=hdom_table_insertrow">向表格添加新行 - 然后添加单元格和内容</a></dt>
</dl>
</div>
