...
<title>relatedTarget 事件属性</title>
...
<div>
<h2>定义和用法</h2>
<p>relatedTarget 事件属性返回与事件的目标节点相关的节点。</p>
<p>对于 mouseover 事件来说，该属性是鼠标指针移到目标节点上时所离开的那个节点。</p>
<p>对于 mouseout 事件来说，该属性是离开目标时，鼠标指针进入的节点。</p>
<p>对于其他类型的事件来说，这个属性没有用。</p>

<h3>语法</h3>
<pre>event.relatedTarget</pre>
</div>

<div>
<h2>实例</h2>
<p>下面例子可返回指针刚刚离开的元素：</p>
...
function getRelElement(event)
  ...
  txt=txt + <code>event.relatedTarget.tagName</code> + &quot; element.&quot;;
  alert(txt);
  }
&lt;/script&gt;
&lt;/head&gt;
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=hdom_event_relatedtarget">relatedtarget</a></dt>
<dd>返回指针刚刚离开的元素（不支持 IE）。</dd>
</dl>
</div>
