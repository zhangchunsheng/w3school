...
<p>scale() 方法为画布的当前变换矩阵添加一个缩放变换。缩放通过独立的水平和垂直缩放因子来完成。例如，传递一个值 2.0 和 0.5 将会导致绘图路径宽度变为原来的两倍，而高度变为原来的 1/2。指定一个负的 <i>sx</i> 值，会导致 X 坐标沿 Y 轴对折，而指定一个负的 <i>sy</i> 会导致 Y 坐标沿着 X 轴对折。</p>
</div>
...
<p><a href="/jsref/met_canvasrenderingcontext2d_rotate.asp" title="HTML DOM rotate() 方法">CanvasRenderingContext2D.rotate()</a></p>
<p><a href="/jsref/met_canvasrenderingcontext2d_translate.asp" title="HTML DOM translate() 方法">CanvasRenderingContext2D.translate()</a></p>
</div>
