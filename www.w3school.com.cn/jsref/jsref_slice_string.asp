...
<h3>语法</h3>
<pre>stringObject.slice(start,end)</pre>
...
<h3>返回值</h3>
<p>一个新的字符串。包括字符串 stringObject 从 start 开始（包括 start）到 end 结束（不包括 end）为止的所有字符。</p>

<h3>说明</h3>
<p>String 对象的方法 slice()、substring() 和 substr() （不建议使用）都可返回字符串的指定部分。slice() 比 substring() 要灵活一些，因为它允许使用负数作为参数。slice() 与 substr() 有所不同，因为它用两个字符的位置来指定子串，而 substr() 则用字符位置和长度来指定子串。</p>
<p>还要注意的是，String.slice() 与 Array.slice() 相似。</p>
</div>
	
<div>
<h2>实例</h2>
<h3>例子 1</h3>
<p>在本例中，我们将提取从位置 6 开始的所有字符：</p>
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=jseg_slice">slice()</a></dt>
<dd>如何使用 slice() 从字符串提取字符。</dd>
</dl>
</div>
