...
<title>HTML DOM getElementById() 方法</title>
...
<h1>HTML DOM getElementById() 方法</h1>
...
<h3>说明</h3>
<p>HTML DOM 定义了多种查找元素的方法，除了 getElementById() 之外，还有 getElementsByName() 和 getElementsByTagName()。</p>
<p>不过，如果您需要查找文档中的一个特定的元素，最有效的方法是 getElementById()。</p>
<p>在操作文档的一个特定的元素时，最好给该元素一个 id 属性，为它指定一个（在文档中）唯一的名称，然后就可以用该 ID 查找想要的元素。</p>
</div>
...
function getValue()
  {
  var x=<code>document.getElementById(&quot;myHeader&quot;)</code>
  alert(x.innerHTML)
  }
&lt;/script&gt;
&lt;/head&gt;
...
<h3>例子 2</h3>
<p>getElementById() 是一个重要的方法，在 DOM 程序设计中，它的使用非常常见。我们为您定义了一个工具函数，这样您就可以通过一个较短的名字来使用 getElementById() 方法了：</p>

<pre>
function id(x) {
  if (typeof x == &quot;string&quot;) return <code>document.getElementById(x)</code>;
  return x;
  }
</pre>

<p>上面这个函数接受元素 ID 作为它们的参数。对于每个这样的参数，您只要在使用前编写 x = id(x) 就可以了。</p>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=hdom_document_getbyid">使用 getElementById()</a></dt>
</dl>
</div>
