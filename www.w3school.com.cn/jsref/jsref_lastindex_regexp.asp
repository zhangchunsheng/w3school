...
<title>JavaScript lastIndex 属性</title>
...
<p>lastIndex  属性用于规定下次匹配的起始位置。</p>

<h3>语法</h3>

<pre>RegExpObject.lastIndex</pre>

<h3>说明</h3>

<p>该属性存放一个整数，它声明的是上一次匹配文本之后的第一个字符的位置。</p>
...
<p class="important"><span>重要事项：</span>不具有标志 g 和不表示全局模式的 RegExp 对象不能使用 lastIndex 属性。</p>

<p class="tip"><span>提示：</span>如果在成功地匹配了某个字符串之后就开始检索另一个新的字符串，需要手动地把这个属性设置为 0。</p>
</div>
...
<p>在下面的例子中，我们将输出匹配完成之后的 lastIndex 属性：</p>
...
patt1.test(str)
document.write(&quot;Match found. index now at: &quot; + <code>patt1.lastIndex</code>);
&lt;/script&gt;
</pre>
</div>
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_lastindex_regexp">lastIndex</a></dt>
<dd>如何使用 lastIndex 属性来检测从何处开始下一次匹配。</dd>
</dl>
</div>
