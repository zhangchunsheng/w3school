...
<title>JavaScript toPrecision() 方法</title>
...
<h1>JavaScript toPrecision() 方法</h1>
...
<div>
<h2>定义和用法</h2>
<p>toPrecision() 方法可在对象的值超出指定位数时将其转换为指数计数法。</p>

<h3>语法</h3>
<pre>NumberObject.toPrecision(num)</pre>

<table class="dataintable"> 
  <tr>
    <th class="no_wrap">参数</th>
    <th>描述</th>
  </tr>  
  <tr>
    <td>num</td>
    <td>必需。规定必须被转换为指数计数法的最小位数。该参数是 1 ~ 21 之间（且包括 1 和 21）的值。有效实现允许有选择地支持更大或更小的 num。如果省略了该参数，则调用方法 toString()，而不是把数字转换成十进制的值。</td>
  </tr>
</table>

<h3>返回值</h3>
<p>返回 NumberObject 的字符串表示，包含 num 个有效数字。如果 num 足够大，能够包括 NumberObject 整数部分的所有数字，那么返回的字符串将采用定点计数法。否则，采用指数计数法，即小数点前有一位数字，小数点后有 num-1 位数字。必要时，该数字会被舍入或用 0 补足。</p>

<h3>抛出</h3>
<p>当 num 太小或太大时抛出异常 RangeError。1 ~ 21 之间的值不会引发该异常。有些实现支持更大范围或更小范围内的值。</p>
<p>当调用该方法的对象不是 Number 时抛出 TypeError 异常。</p>
</div>
	
<div>
<h2>实例</h2>
<p>在本例中，我们将把一个数字转换为指数计数法：</p>
<pre>Show 10,000 as an exponential notation:
&lt;script type=&quot;text/javascript&quot;&gt;
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=jseg_toprecision">toPrecision()</a></dt>
<dd>如何使用 toPrecision() 来把数字转换为指数计数法。</dd>
</dl>
</div>
