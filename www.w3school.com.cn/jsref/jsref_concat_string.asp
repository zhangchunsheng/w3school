...
<pre>stringObject.concat(stringX,stringX,...,stringX)</pre>
...
<p>concat() 方法将把它的所有参数转换成字符串，然后按顺序连接到字符串 stringObject 的尾部，并返回连接后的字符串。请注意，stringObject 本身并没有被更改。</p>

<p>stringObject.concat() 与 Array.concat() 很相似。</p>
</div>

<div>
<h2>提示和注释</h2>

<p class="tip"><span>提示：</span>请注意，使用 &quot; + &quot; 运算符来进行字符串的连接运算通常会更简便一些。</p>
</div>
...
<p>在本例中，我们将创建两个字符串，然后使用 concat() 把它们显示为一个字符串：</p>
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_concat_string">concat()</a></dt>
<dd>如何使用 concat() 来连接两个字符串。</dd>
</dl>
</div>
