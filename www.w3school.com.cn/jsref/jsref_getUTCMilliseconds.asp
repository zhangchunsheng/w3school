...
<title>JavaScript getUTCMilliseconds() 方法</title>
...
<h1>JavaScript getUTCMilliseconds() 方法</h1>
...
<p>getUTCMilliseconds() 方法可根据世界时 (UTC) 返回时间的毫秒。</p>

<h3>语法</h3>

<pre>dateObject.getUTCMilliseconds()</pre>

<h3>返回值</h3>

<p>当 dateObject 用世界时表示时，返回它的毫秒字段，该值是一个 0 ~ 999 之间的整数。</p>
</div>
...
<p class="note"><span>注释：</span>由 getUTCMilliseconds() 返回的值是一个三位的数字。不过返回值不总是三位的，如果该值小于 100，则仅返回两位数字，如果该值小于 10，则仅返回一位数字。</p>
...
<p>在此处，我们将从具体的日期和时间中提取 UTC 毫秒：</p>
...
<p class="note"><span>注释：</span>上面的代码将把毫秒设置为 1，这是由于在日期中没有定义毫秒。</p>
</div>
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_getUTCmilliseconds">getUTCMilliseconds()</a></dt>
<dd>如何使用 getUTCMilliseconds() 来取得当前的 UTC 毫秒。</dd>
<dt><a href="/tiy/t.asp?f=jseg_datetime_UTC">显示当前的 UTC 时间</a></dt>
<dd>如何使用 getUTCHours() 显示当前的 UTC 时间。</dd>
<dt><a href="/tiy/t.asp?f=jseg_datetime_UTC_2">显示当前的 UTC 时间（带有一个判断数值是否小于 10 的函数） </a></dt>
<dd>如何使用 getUTCHours(), getUTCMinutes() 以及 getUTCSeconds() 来显示当前的 UTC 时间。</dd>
</dl>
</div>
