...
<p class="tip"><span>提示：</span>请使用 isNaN() 全局函数来判断一个值是否是 NaN 值。</p>
...
<p>Number.NaN 是一个特殊值，说明某些算术运算（如求负数的平方根）的结果不是数字。方法 parseInt() 和 parseFloat() 在不能解析指定的字符串时就返回这个值。对于一些常规情况下返回有效数字的函数，也可以采用这种方法，用 Number.NaN 说明它的错误情况。</p>

<p>JavaScript 以 NaN 的形式输出 Number.NaN。请注意，NaN 与其他数值进行比较的结果总是不相等的，包括它自身在内。因此，不能与 Number.NaN 比较来检测一个值是不是数字，而只能调用 isNaN() 来比较。</p>

<p>在 ECMAScript v1 和其后的版本中，还可以用<a href="/jsref/jsref_nan.asp" title="JavaScript NaN 属性">预定义的全局属性 NaN</a> 代替 Number.NaN。</p>
</div>
...
<p><a href="/tiy/t.asp?f=jseg_nan_number">TIY</a></p>
</div>
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_nan_number">NaN</a></dt>
...
<p>JavaScript 全局对象参考手册：<a href="/jsref/jsref_nan.asp" title="JavaScript NaN 属性">NaN 属性</a></p>

<p>JavaScript 全局对象参考手册：<a href="/jsref/jsref_isNaN.asp" title="JavaScript isNaN() 函数">isNaN() 函数</a></p>
</div>
