...
<title>JavaScript substr() 方法</title>
...
<p>substr() 方法可在字符串中抽取从 <i>start</i> 下标开始的指定数目的字符。</p>

<h3>语法</h3>
<pre><i>stringObject</i>.substr(<i>start</i>,<i>length</i>)</pre>
...
<table class="dataintable"> 
  ...
  <tr>
    <td><i>length</i></td>
    <td>可选。子串中的字符数。必须是数值。如果省略了该参数，那么返回从 <i>stringObject</i> 的开始位置到结尾的字串。</td>
  </tr>
</table>

<h3>返回值</h3>
<p>一个新的字符串，包含从 <i>stringObject</i> 的 <i>start</i>（包括 start 所指的字符） 处开始的 <i>length</i> 个字符。如果没有指定 <i>length</i>，那么返回的字符串包含从 <i>start</i> 到 <i>stringObject</i> 的结尾的字符。</p>
</div>


<div>
<h2>提示和注释</h2>
<p class="note"><span>注释：</span>substr() 的参数指定的是子串的开始位置和长度，因此它可以替代 substring() 和 slice() 来使用。</p>
<p class="important"><span>重要事项：</span>ECMAscript 没有对该方法进行标准化，因此反对使用它。</p>
<p class="important"><span>重要事项：</span>在 IE 4 中，参数 <i>start</i> 的值无效。在这个 BUG 中，<i>start</i> 规定的是第 0 个字符的位置。在之后的版本中，此 BUG 已被修正。</p>
</div>
...
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=jseg_substr">substr()</a></dt>
<dd>如何使用 substr() 从字符串提取一些字符。</dd>
</dl>
</div>
