...
<title>HTML DOM onsubmit 事件句柄</title>
...
<h3>语法</h3>
<pre>form.onsubmit</pre>

<h3>说明</h3>
<p>Form 对象的 onsubmit 属性指定了一个事件句柄函数。当用户单击了表单中的 Submit 按钮而提交一个表单时，就会调用这个事件句柄函数。注意，当调用方法Form.submit() 时，该处理器函数不会被调用。</p>
<p>如果 onsubmit 句柄返回 fasle，表单的元素就不会提交。如果该函数返回其他值或什么都没有返回，则表单会被提交。</p>
</div>

<div>
<h2>提示和注释</h2>
<p class="tip"><span>提示：</span>由于 onsubmit 句柄可以取消表单的提交，所以它对于进行表单验证是十分理想的。</p>
</div>
