...
<title>JavaScript parseInt() 函数</title>
...
<p>当参数 <i>radix</i> 的值为 0，或没有设置该参数时，parseInt() 会根据 <i>string</i> 来判断数字的基数。</p>

<p>举例，如果 <i>string</i> 以 &quot;0x&quot; 开头，parseInt() 会把 <i>string</i> 的其余部分解析为十六进制的整数。如果 <i>string</i> 以 0 开头，那么 ECMAScript v3 允许 parseInt() 的一个实现把其后的字符解析为八进制或十六进制的数字。如果 <i>string</i> 以 1 ~ 9 的数字开头，parseInt() 将把它解析为十进制的整数。</p>
</div>
...
<p class="note"><span>注释：</span>只有字符串中的第一个数字会被返回。</p>
...
<p>在本例中，我们将使用 parseInt() 来解析不同的字符串：</p>
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_parseInt">parseInt()</a></dt>
<dd>如何使用 parseInt() 来解析不同的字符串。</dd>
</dl>
</div>
