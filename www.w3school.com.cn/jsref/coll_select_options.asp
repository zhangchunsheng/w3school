...
<title>HTML DOM options 集合</title>
...
<h1>HTML DOM options 集合</h1>

<div class="backtoreference">
<p><a href="/jsref/dom_obj_select.asp">HTML DOM Select 对象</a></p>
</div>

<div>
<h2>定义和用法</h2>
<p>option 集合可返回包含 &lt;select&gt; 元素中所有 &lt;option&gt; 的一个数组。</p>
<p class="note"><span>注释：</span>数组中的每个元素对应一个 &lt;option&gt; 标签 - 由 0 起始。</p>

<h3>语法</h3>
<pre>selectObject.options[]</pre>

<h3>说明</h3>
<p>options[] 集合并非一个普通的 HTMLcollection。为了和早期的浏览器向后兼容，这个集合有某种特殊的行为：允许通过 Select 对象来改变显示的选项：</p>
<ul>
<li>如果把 options.length 属性设置为 0,Select 对象中所有选项都会被清除。</li>
<li>如果 options.length 属性的值比当前值小，出现在数组尾部的元素就会被丢弃。</li>
<li>如果把 options[] 数组中的一个元素设置为 null，那么选项就会从 Select 对象中删除。</li>
<li>可以通过构造函数 Option() 来创建一个新的 option 对象（需要设置 options.length 属性）。</li>
</ul>
</div>

<div>
<h2>实例</h2>
<p>下面的例子返回下拉列表中所有选项的文本：</p>
<pre>
&lt;html&gt;
&lt;head&gt;
&lt;script type=&quot;text/javascript&quot;&gt;
...
<p><a href="/tiy/t.asp?f=hdom_select_options">亲自试一试</a></p>
</div>

<div class="example">
<h2>TIY</h2>
<dl>
<dt><a href="/tiy/t.asp?f=hdom_select_options">输出下拉列表中所有选项的文本</a></dt>
<dt><a href="/tiy/t.asp?f=hdom_option_settext">设置下拉列表中被选选项的文本</a></dt>
</dl>
</div>
