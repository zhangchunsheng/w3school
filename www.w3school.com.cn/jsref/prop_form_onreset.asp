...
<title>HTML DOM onreset 事件句柄</title>
...
<h3>语法</h3>
<pre>form.onreset</pre>

<h3>说明</h3>
<p>Form 对象的 onreset 属性指定了一个事件句柄函数。当用户单击了表单中的 Reset 按钮而提交一个重置时，就会调用这个事件句柄函数。注意，这个句柄不会作为 Form.reset() 方法响应而被调用。如果 onreset 句柄返回 fasle，表单的元素就不会重置。</p>
</div>
