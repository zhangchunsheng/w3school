...
<title>HTML DOM onchange 事件句柄</title>
...
<div>
<h2>定义和用法</h2>
<p>select() 方法用于选择该元素中的文本。</p>

<h3>语法</h3>
<pre>textareaObject.onchange</pre>

<h3>说明</h3>
<p>Textarea 的 onchange 属性引用了一个事件句柄函数，当用户改变文本区域的值然后通过把键盘焦点移动到其他地方“确认”这些改变的时候，调用该句柄。</p>
<p>请注意，当一个 Text 对象的 value 属性通过 JavaScript 设置的时候，onchange 事件句柄不会调用。还要注意，该句柄是为了处理对输入值的一次完整改变，因此，它不会在逐个按键的基础上调用。</p>
</div>
