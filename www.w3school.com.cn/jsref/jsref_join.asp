...
<title>JavaScript join() 方法</title>
...
<pre>arrayObject.join(<i>separator</i>)</pre>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>separator</td>
    <td>可选。指定要使用的分隔符。如果省略该参数，则使用逗号作为分隔符。</td>
  </tr>
</table>

<h3>返回值</h3>

<p>返回一个字符串。该字符串是通过把 arrayObject 的每个元素转换为字符串，然后把这些字符串连接起来，在两个元素之间插入 <i>separator</i> 字符串而生成的。</p>
</div>
...
<p>在本例中，我们将创建一个数组，然后把它的所有元素放入一个字符串：</p>
...
document.write(<code>arr.join()</code>)
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_join">join()</a></dt>
<dd>如何使用 join() 从数组创建字符串。</dd>
</dl>
</div>
