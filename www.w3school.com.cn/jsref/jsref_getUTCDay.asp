...
<title>JavaScript getUTCDay() 方法</title>
...
<p>getUTCDay() 方法根据世界时返回表示星期的一天的一个数字。</p>

<h3>语法</h3>

<pre>dateObject.getUTCDay()</pre>

<h3>返回值</h3>

<p>dateObject 用世界时表示时，返回该星期中的某一天 ，该值是 0（星期天） ~ 6（星期六） 中的一个值。</p>
</div>
...
<p>在本例中，我们将取得本周的当前 UTC 天（以一个数字）：</p>
...
<p>现在，我将创建一个数组，使得上面的例子能够输出星期的名称，而不仅仅是一个数字：</p>
...
<dl>
<dt><a href="/tiy/t.asp?f=jseg_date_weekday_UTC">getUTCDay()</a></dt>
<dd>使用 getUTCDay() 和一个数组来输出星期的名称，而不是一个数字。</dd>
</dl>
</div>
