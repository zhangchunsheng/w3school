...
<p>bottom 属性规定元素的底部边缘。该属性定义了定位元素下外边距边界与其包含块下边界之间的偏移。</p>

<p class="note"><span>注释：</span>如果 &quot;position&quot; 属性的值为 &quot;static&quot;，那么设置 &quot;bottom&quot; 属性不会产生任何效果。</p>

<h3>说明</h3>

<p>对于 static 元素，为 auto；对于长度值，则为相应的绝对长度；对于百分比数值，为指定值；否则为 auto。</p>

<p>对于相对定义元素，如果 bottom 和 top 都是 auto，其计算值则都是 0；如果其中之一为 auto，则取另一个值的相反数；如果二者都不是 auto，bottom 将取 top 值的相反数。</p>
...
<table class="dataintable">
  ...
  <tr>
    <th>JavaScript 语法：</th>
    <td><i>object</i>.style.bottom=&quot;50px&quot;</td>
  </tr>
</table>
</div>
...
<p>把图像的底边缘设置在其包含元素底边缘之上 5 像素高的位置：</p>

<pre>
img
  {
  position:absolute;
  <code>bottom:5px;</code>
  }
</pre>

<p><a href="/tiy/t.asp?f=csse_position_bottom_test">TIY</a></p>
</div>
...
<tr>
<td>auto</td>
<td>默认值。通过浏览器计算底部的位置。</td>
</tr>

<tr>
<td><i>%</i></td>
<td>设置以包含元素的百分比计的底边位置。可使用负值。</td>
</tr>

<tr>
<td><i>length</i></td>
<td>使用 px、cm 等单位设置元素的底边位置。可使用负值。</td>
</tr>

<tr>
<td>inherit</td>
<td>规定应该从父元素继承 bottom 属性的值。</td>
</tr>
</table>
</div>
...
<p>HTML DOM 参考手册：<a href="/jsref/prop_style_bottom.asp" title="HTML DOM bottom 属性">bottom 属性</a></p>
</div>
