...
<title>HTML5 canvas clip() 方法</title>
...
<p>从画布中剪切 200*120 像素的矩形区域。然后，绘制绿色矩形。只有被剪切区域内的绿色矩形部分是可见的：</p>
...
var c=document.getElementById(&quot;myCanvas&quot;);
var ctx=c.getContext(&quot;2d&quot;);
<span class="code_comment">// 剪切矩形区域</span>
ctx.rect(50,20,200,120);
ctx.stroke();
ctx.clip();
<span class="code_comment">// 在 clip() 之后绘制绿色矩形</span>
ctx.fillStyle=&quot;green&quot;;
ctx.fillRect(0,0,150,100);
</pre>

<p><a target="_blank" href="/tiy/t.asp?f=html5_canvas_clip">亲自试一试</a></p>
</div>
...
<p>Internet Explorer 9、Firefox、Opera、Chrome 以及 Safari 支持 clip() 方法。</p>
...
<p class="tip"><span>提示：</span>一旦剪切了某个区域，则所有之后的绘图都会被限制在被剪切的区域内（不能访问画布上的其他区域）。您也可以在使用 clip() 方法前通过使用 save() 方法对当前画布区域进行保存，并在以后的任意时间对其进行恢复（通过 restore() 方法）。</p>
