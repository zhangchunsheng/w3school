...
<title>HTML5 canvas transform() 方法</title>
...
<h1>HTML5 canvas transform() 方法</h1>
...
<code class="code_comment">ctx.transform(1,0.5,-0.5,1,30,10);</code>
ctx.fillStyle=&quot;red&quot;;
ctx.fillRect(0,0,250,100);
...
<p><a target="_blank" href="/tiy/t.asp?f=html5_canvas_transform">亲自试一试</a></p>
</div>
...
<p>Internet Explorer 9、Firefox、Opera、Chrome 以及 Safari 支持 transform() 方法。</p>
...
<p>transform() 方法替换当前的变换矩阵。它以下面描述的矩阵来操作当前的变换矩阵：</p>
...
<p class="note"><span>注释：</span>该变换只会影响 transform() 方法调用之后的绘图。</p>

<p class="note"><span>注释：</span>transform() 方法的行为相对于由 rotate(), scale(), translate(), or transform() 完成的其他变换。例如：如果您已经将绘图设置为放到两倍，则 transform() 方法会把绘图放大两倍，您的绘图最终将放大四倍。</p>

<p class="tip"><span>提示：</span>请查看 <a href="/html5/canvas_settransform.asp" title="HTML canvas setTransform() 方法">setTransform()</a> 方法，它不会相对于其他变换来发生行为。</p>

<h3>JavaScript 语法：</h3>

<pre>context.transform(<i>a</i>,<i>b</i>,<i>c</i>,<i>d</i>,<i>e</i>,<i>f</i>);</pre>
