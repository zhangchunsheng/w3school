...
<li class="currentLink"><a href="/ado/ado_sort.asp" title="ADO 排序">ADO 排序</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/ado/ado_query.asp" title="ADO 查询">ADO 查询</a></li>
<li class="next"><a href="/ado/ado_add.asp" title="ADO 添加记录">ADO 添加</a></li>
</ul>
</div>

<div>
<p><strong>我们可以使用SQL来规定如何对记录集中的数据进行排序。</strong></p>
</div>

<div class="example">
<h2>实例</h2>
<dl>
<dt><a href="/tiy/s.asp?f=demo_adoe_sort_1">根据指定的字段名处对记录进行升序排序</a></dt>
<dd>如何根据指定字段名对数据进行排序</dd>

<dt><a href="/tiy/s.asp?f=demo_adoe_sort_2">根据指定的字段名处对记录进行降序排序</a></dt>
<dd>如何根据指定字段名对数据进行排序</dd>

<dt><a href="/tiy/s.asp?f=demo_adoe_sort_3">让用户来选择根据哪列进行排序</a></dt>
<dd>让用户来选择根据哪列进行排序</dd>
</dl>
</div>

<div>
<h2>对数据进行排序</h2>
<p>我们希望显示 &quot;Customers&quot; 表中的&quot;Companyname&quot;和&quot;Contactname&quot;字段，并根据&quot;Companyname&quot;进行排序（请记得用.asp为后缀保存）：</p>
<pre>
&lt;html&gt;
&lt;body&gt;
...
<code>sql=&quot;SELECT Companyname, Contactname FROM
Customers ORDER BY CompanyName&quot;</code>
rs.Open sql, conn
%&gt;
