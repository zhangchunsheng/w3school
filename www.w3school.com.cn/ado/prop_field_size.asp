...
<title>ADO DefinedSize 和 ActualSize 属性</title>
</head>
...
<h1>ADO DefinedSize 和 ActualSize 属性</h1>
...
<div>
<h2>定义和用法</h2>
<p>DefinedSize 属性会返回一个 long 值，该值指示一个字段被定义的长度（字节）。</p>

<p>ActualSize 属性是一个只读属性。它会返回一个 long 值，该值指示某个字段的值的实际长度。如果 ADO 无法确定该 Field 对象的值的长度，则返回 adUnknown。</p>

<p>使用 DefinedSize 属性可确定 Field 对象的数据容量，而 ActualSize 则表明其实际长度。</p>

<h3>语法</h3>
<pre>objrs.Fields(number).DefinedSize 
objrs.Fields(number).ActualSize</pre>
</div>
