...
<div>
<h2>定义和用法</h2>
<p>Open 方法可打开一个到数据源的连接。当连接打开时，您可以对数据源执行命令。</p>

<h3>语法</h3>
<pre>connection.Open connectionstring,userID,password,options</pre>
...
<table class="dataintable"> 
  ...
  <tr>
    ...
    <td>
	<p>可选。一个包含有关连接的信息的字符串值。该字符串由一系列被分号隔开的 parameter=value 语句组成的。</p>
    <p>有关有效设置的详细信息，请参阅 <a href="/ado/prop_conn_connectionstring.asp">ConnectionString</a> 属性。</p>
    </td>
  </tr>
  <tr>
    <td>userID</td>
    <td>可选。一个字符串值，包含建立连接时要使用的用户名称。</td>
  </tr>
  <tr>
    <td>password</td>
    <td>可选。一个字符串值，包含建立连接时要使用的密码。</td>
  </tr>
  <tr>
    <td>options</td>
    <td>可选。一个 <a href="#connectoptionenum">ConnectOptionEnum</a> 值，确定应在建立连接之后（同步）还是应在建立连接之前（异步）返回本方法。</td>
  </tr>
</table>
</div>
...
<p>带有 userID 和 password 的 DSN-less 连接（请把 conn.open 语句写在一行中）：</p>
<pre>&lt;%
set conn=Server.CreateObject(&quot;ADODB.Connection&quot;)
...
<div>
<h3><a id="connectoptionenum">ConnectOptionEnum</a></h3>
<table class="dataintable">
    <tr>
      <th>常数</th>
      <th>值</th>
    ...
    <tr>
      ...
      <td>16</td>
      <td>异步打开连接。ConnectComplete 事件可以用来确定连接何时可用。</td>
    </tr>
</table>
</div>
