...
<div>
<h2>定义和用法</h2>
<p>Size 属性可设置或返回一个 long 值，该值指示 Parameter 对象中的值的最大尺寸（字节或字符）。</p>


<h3>语法</h3>
<pre>objparameter.Size</pre>

<h3>说明</h3>
<p>如果要指定 Parameter 对象的变长数据类型（例如所有 String 类型，如 adVarChar），必须先设置对象的 Size 属性，然后再将该对象追加到 Parameters 集合。否则，将会产生错误。</p>

<p>如果已经将 Parameter 对象追加到 Command 对象的 Parameters 集合中，并将其类型更改为变长数据类型，则必须先设置 Parameter 对象的 Size 属性，然后再执行 Command 对象。否则，将发生错误。</p>

<p>如果使用 Refresh 方法从提供者获取参数信息，并且返回一个或多个变长数据类型 Parameter 对象，ADO 可能会根据其可能的最大尺寸为这些参数分配内存空间，这在执行过程中可能会导致错误。若要避免错误，在执行命令前应显式地设置这些参数的 Size 属性。</p>
