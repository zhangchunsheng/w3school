...
<div>
<h2>定义和用法</h2>
<p>Cancel 方法可取消方法调用的执行。</p>
<p>Cancel 方法可为不同的对象终止不同的任务。下面的表格列出了当此方法被调用时被终止的任务：</p>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>Command </td>
    <td>Execute。
      <p>注释：在 Cancel 方法被调用之前，Execute 方法的 Options 参数必须被设置为 adAsyncExecute 或者 adAsyncFetch，否则会发生 run-time 错误。</p>
    </td>
  </tr>
  <tr>
    <td>Connection</td>
    <td>Execute 或 Open。
      <p>注释：在 Cancel 方法被调用之前，Open 方法的 Options 参数必须被设置为 adSyncConnect，Execute 方法的 Options 参数必须被设置为 adAsyncExecute 或者 adAsyncFetch，否则会发生 run-time 错误。</p>
    </td>
  </tr>
  <tr>
    <td>Record</td>
    <td>CopyRecord, DeleteRecord, MoveRecord, 或者 Open。</td>
  </tr>
  <tr>
    <td>Recordset</td>
    <td>Open</td>
