...
<li class="currentLink"><a href="/ado/ado_recordset.asp" title="ADO Recordset（记录集）">ADO 记录集</a></li>
...
<h1>ADO Recordset（记录集）</h1>

<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/ado/ado_connect.asp" title="ADO 数据库连接">ADO 连接</a></li>
<li class="next"><a href="/ado/ado_display.asp" title="ADO 显示">ADO 显示</a></li>
</ul>
</div>

<div>
<p><strong>如需读取数据库的数据，那么其中的数据必须首先被载入一个记录集中。</strong></p>
</div>

<div>
<h2>创建一个 ADO 表记录集（ADO Table Recordset）</h2>
<p>在 ADO 数据库连接创建之后，如上一章所述，接下来就可以建立一个 ADO 记录集了。</p>
<p>假设我们有一个名为 &quot;Northwind&quot; 的数据库，我们可以通过下面的代码访问数据库中的 &quot;Customers&quot; 表：</p>
<pre>
&lt;%
...
<div>
<h2>创建一个 ADO SQL 记录集 （ADO SQL Recordset）</h2>
<p>我们也可使用 SQL 访问 &quot;Customers&quot; 表中的数据：</p>
<pre>
&lt;%
set conn=Server.CreateObject(&quot;ADODB.Connection&quot;)
conn.Provider=&quot;Microsoft.Jet.OLEDB.4.0&quot;
...
<div>
<h2>从记录集中提取数据</h2>
<p>在记录集被打开后，我们可以从记录集中提取数据。</p>
<p>假设我们用一个名为 &quot;Northwind&quot; 的数据库，我们可以通过下面的代码访问数据库中 &quot;Customers&quot; 表：</p>
<pre>
&lt;%
set conn=Server.CreateObject(&quot;ADODB.Connection&quot;)
conn.Provider=&quot;Microsoft.Jet.OLEDB.4.0&quot;
...
<div>
<h2>ADO 记录集对象（ADO Recordset Object）</h2>
<p>ADO Recordset 对象可被用来容纳来自数据库表的记录集。</p>
<p>查看 ADO Recordset 对象的所有方法和属性。</p>
</div>
