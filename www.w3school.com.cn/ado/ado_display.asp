...
<li class="currentLink"><a href="/ado/ado_display.asp" title="ADO 显示">ADO 显示</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/ado/ado_recordset.asp" title="ADO Recordset（记录集）">ADO 记录集</a></li>
<li class="next"><a href="/ado/ado_query.asp" title="ADO 查询">ADO 查询</a></li>
</ul>
</div>

<div>
<p><strong>显示来自记录集中的数据的最常用的方法，就是把数据显示在 HTML 表格中。</strong></p>
</div>
...
<dd>如何首先创建一个数据库连接，然后创建一个记录集，然后把其中的数据显示在HTML中。</dd>

<dt><a href="/tiy/s.asp?f=demo_adoe_display_2">在 HTML 表格中显示记录</a></dt>
<dd>如何把数据表中的数据显示在HTML表格中。</dd>

<dt><a href="/tiy/s.asp?f=demo_adoe_display_3">向 HTML 表格添加标题</a></dt>
<dd>如何向HTML表格添加标题，以使其可读性更强。</dd>

<dt><a href="/tiy/s.asp?f=demo_adoe_display_4">向 HTML 表格添加颜色</a></dt>
<dd>如何向HTML表格添加颜色，以使其更加美观。</dd>
</dl>
</div>

<div>
<h2>显示字段名称和字段值</h2>
<p>我们有一个名为 &quot;Northwind&quot; 的数据库，并且我们希望显示出 &quot;Customers&quot; 表中的数据（记得以 .asp 为扩展名来保存这个文件）：</p>
<pre>
&lt;html&gt;
&lt;body&gt;
...
<div>
<h2>在一个 HTML 表格中显示字段名称和字段的值</h2>
<p>我们也可以通过下面的代码把表 &quot;Customers&quot; 中的数据显示在一个 HTML 表格中：</p>
<pre>
&lt;html&gt;
&lt;body&gt;
...
<code>&lt;table border=&quot;1&quot; width=&quot;100%&quot;&gt;</code>
&lt;%do until rs.EOF%&gt;
   <code>&lt;tr&gt;</code>
   &lt;%for each x in rs.Fields%&gt;
      <code>&lt;td&gt;&lt;%Response.Write(x.value)%&gt;&lt;/td&gt;</code>
   &lt;%next
   rs.MoveNext%&gt;
   <code>&lt;/tr&gt;</code>
&lt;%loop
...
<div>
<h2>向 HTML 表格添加标题</h2>
<p>我们希望为这个 HTML 表格添加标题，这样它就更易读了：</p>
<pre>
&lt;html&gt;
&lt;body&gt;
