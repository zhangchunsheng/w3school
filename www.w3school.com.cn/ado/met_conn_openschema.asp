...
<div>
<h2>定义和用法</h2>
<p>OpenSchema 方法可返回 Recordset 对象，该对象包含有关数据源的模式信息。举例，schema 信息可包括表的名称，表中的列名，每列的数据类型。Recordset 将以只读、静态游标模式打开。</p>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>querytype</td>
    <td>
	<p>必需。任意 <a href="/ado/app_schemaenum.asp">SchemaEnum</a> 值，表示要运行的模式查询的类型。</p>
	<p>注释：OLEDB 规范只要求三个 SchemaEnum 值得到支持：adSchemaTables, adSchemaColumns 以及  adSchemaProviderTypes。</p>
	</td>
  </tr>
  <tr>
    <td>criteria</td>
    <td>可选。每个 QueryType 选项的查询约束的数组，如 SchemaEnum 中列出的内容。</td>
  </tr>
  <tr>
    <td>schemaid</td>
    <td>OLE DB 规范未定义的提供者模式查询的 GUID。如果 QueryType 被设置为 adSchemaProviderSpecific，则需要此参数。否则，将不使用它。</td>
  </tr>
</table>
</div>
