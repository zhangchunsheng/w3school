...
<p>Execute 方法可执行 Command 对象的 CommandText 属性中指定的查询、SQL 语句或存储过程。</p>

<p>如果 CommandText 属性指定以行返回的查询，执行产生的任何结果都将存储在新的 Recordset 对象中。如果此命令不是以行返回的查询，提供者将返回关闭的 Recordset 对象。</p>

<h3>语法：对于以行返回的 Command：</h3>
<pre>Set rs=objcommand.Execute(ra,parameters,options)</pre>

<h3>语法：对于不是以行返回的 Command：</h3>
<pre>objcommand.Execute ra,parameters,options</pre>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>ra</td>
    <td>可选。返回受查询影响的记录的数目。对于以行返回的查询，请使用 Recordset 对象的 RecordCount 属性来计算该对象中的记录数量。</td>
  </tr>
  <tr>
    <td>parameters</td>
    <td>可选。用 SQL 语句传递的参数值。用于更改、更新或向 Parameters 集合插入新的参数值。</td>
  </tr>
  <tr>
    <td>options</td>
    <td>可选。指示提供者应如何计算 Command 对象的 CommandText 属性。可以是一个或者多个 <a href="/ado/app_commandtypeenum.asp">CommandTypeEnum</a> 或 <a href="/ado/app_executeoptionenum.asp">ExecuteOptionEnum</a> 值。默认是 adCmdUnspecified。</td>
  </tr>
</table>
</div>
