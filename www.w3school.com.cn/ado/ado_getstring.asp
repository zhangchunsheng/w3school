...
<title>ADO 通过 GetString() 加速脚本</title>
</head>
...
<li class="currentLink"><a href="/ado/ado_getstring.asp" title="ADO 通过 GetString() 加速脚本">ADO 加速</a></li>
</ul>
<h2>ADO 对象</h2>
<ul>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/ado/ado_delete.asp" title="ADO 删除记录">ADO 删除</a></li>
<li class="next"><a href="/ado/ado_ref_command.asp" title="ADO Command 对象">ADO Command</a></li>
</ul>
</div>

<div>
<p><strong>请使用 GetString() 方法来加速您的 ASP 脚本（来代替多行的 Response.Write）。</strong></p>
</div>

<div id="example">
<h2>实例</h2>
<dl>
<dt><a href="/tiy/s.asp?f=demo_adoe_display_5">使用 GetString()</a></dt>
<dd>如何使用 GetString() 在 HTML 表格中显示记录集中的数据。</dd>
</dl>
</div>
...
<p>下面的例子演示了在 HTML 表格中显示数据库查询的一种方法：</p>
<pre>
&lt;html&gt;
&lt;body&gt;
...
<p>GetString() 方法使我们有能力仅使用一次 Response.Write，就可以显示所有的字符串。同时它甚至不需要 do..loop 代码以及条件测试来检查记录集是否处于 EOF。</p>
...
<ul>
<li>coldel - 用作列分隔符的 HTML</li>
<li>rowdel - 用作行分隔符的 HTML</li>
<li>nullexpr - 当列为空时所使用的 HTML</li>
</ul>
<p class="note"><span>注释：</span>GetString() 方法是 ADO 2.0 的特性。您可从下面的地址下载 ADO 2.0：<a href="http://www.microsoft.com/data/download.htm">http://www.microsoft.com/data/download.htm</a></p>

<p>在下面的例子中，我们将使用 GetString() 方法，把记录集存为一个字符串：</p>
<pre>&lt;html&gt;
&lt;body&gt;
...
<p>上面的变量 str 包含着由 SELECT 语句返回的所有列和行的一个字符串。在每列之间会出现 &lt;/td&gt;&lt;td&gt;，在每行之间会出现 &lt;/td&gt;&lt;/tr&gt;&lt;tr&gt;&lt;td&gt;。这样，仅使用一次 Response.Write，我们就得到了需要的 HTML。</p>
</div>
