...
<title>SQL Server DATEPART() 函数</title>
</head>
...
<p>DATEPART() 函数用于返回日期/时间的单独部分，比如年、月、日、小时、分钟等等。</p>

<h3>语法</h3>
<pre>DATEPART(<i>datepart</i>,<i>date</i>)</pre>

<p><i>date</i> 参数是合法的日期表达式。<i>datepart</i> 参数可以是下列的值：</p>
...
<pre>
SELECT <code>DATEPART(yyyy,OrderDate)</code> AS OrderYear,
<code>DATEPART(mm,OrderDate)</code> AS OrderMonth,
<code>DATEPART(dd,OrderDate)</code> AS OrderDay
FROM Orders
WHERE OrderId=1
</pre>
