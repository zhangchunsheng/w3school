...
<li class="currentLink"><a href="/sql/sql_like.asp" title="SQL LIKE 运算符">SQL Like</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/sql/sql_top.asp" title="SQL TOP 子句">SQL Top</a></li>
<li class="next"><a href="/sql/sql_wildcards.asp" title="SQL 通配符">SQL 通配符</a></li>
</ul>
</div>
...
WHERE City LIKE 'N%'</pre>

<p class="tip"><span>提示：</span>&quot;%&quot; 可用于定义通配符（模式中缺少的字母）。</p>
...
<p>通过使用 NOT 关键字，我们可以从 &quot;Persons&quot; 表中选取居住在<em>不包含</em> &quot;lon&quot; 的城市里的人：</p>

<p>我们可以使用下面的 SELECT 语句：</p>

<pre>SELECT * FROM Persons
WHERE City NOT LIKE '%lon%'</pre>
