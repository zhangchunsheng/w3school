...
<title>SQL Server DATEADD() 函数</title>
</head>
...
<h3>语法</h3>
<pre>DATEADD(<i>datepart</i>,<i>number</i>,<i>date</i>)</pre>

<p><i>date</i> 参数是合法的日期表达式。<i>number</i> 是您希望添加的间隔数；对于未来的时间，此数是正数，对于过去的时间，此数是负数。</p>

<p><i>datepart</i> 参数可以是下列的值：</p>
...
<pre>
SELECT OrderId,<code>DATEADD(day,2,OrderDate)</code> AS OrderPayDate
FROM Orders
</pre>
