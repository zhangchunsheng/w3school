...
<title>SQL CREATE INDEX 语句</title>
</head>
...
<li class="currentLink"><a href="/sql/sql_create_index.asp" title="SQL CREATE INDEX 语句">SQL Create Index</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/sql/sql_default.asp" title="SQL DEFAULT 约束">SQL Default</a></li>
<li class="next"><a href="/sql/sql_drop.asp" title="SQL 撤销索引、表以及数据库">SQL Drop</a></li>
</ul>
</div>
...
<p class="note"><span>注释：</span>更新一个包含索引的表需要比更新一个没有索引的表更多的时间，这是由于索引本身也需要更新。因此，理想的做法是仅仅在常常被搜索的列（以及表）上面创建索引。</p>


<h3>SQL CREATE INDEX 语法</h3>
...
<p class="note"><span>注释：</span>&quot;column_name&quot; 规定需要索引的列。</p>


<h3>SQL CREATE UNIQUE INDEX 语法</h3>
