...
<li class="currentLink"><a href="/sql/sql_func_avg.asp" title="SQL AVG() 函数">SQL avg()</a></li>
...
<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="/sql/sql_functions.asp" title="SQL 函数">SQL functions</a></li>
<li class="next"><a href="/sql/sql_func_count.asp" title="SQL COUNT() 函数">SQL count()</a></li>
</ul>
</div>
...
<p>现在，我们希望计算 &quot;OrderPrice&quot; 字段的平均值。</p>

<p>我们使用如下 SQL 语句：</p>

<pre>SELECT AVG(OrderPrice) AS OrderAverage FROM Orders</pre>
...
<p>现在，我们希望找到 OrderPrice 值高于 OrderPrice 平均值的客户。</p>

<p>我们使用如下 SQL 语句：</p>

<pre>SELECT Customer FROM Orders
WHERE OrderPrice&gt;(SELECT AVG(OrderPrice) FROM Orders)</pre>
